...
static inline int
interval_counter_init(struct interval_counter *counter, uint32_t now, uint32_t max_timeout, struct memory_context *mctx);
...
static inline void
interval_counter_put(struct interval_counter *counter, uint32_t from, uint32_t timeout, int32_t cnt);
...
interval_counter_init(struct interval_counter *counter, uint32_t now, uint32_t max_timeout, struct memory_context *mctx) {
    ...
    counter->values = memory_balloc(mctx, (size_t)counter->range_size * sizeof(*counter->values));
    if (counter->values == NULL) {
        return -1;
    }
    memset(counter->values, 0, (size_t)counter->range_size * sizeof(*counter->values));
...
static inline void
interval_counter_free(struct interval_counter *counter) {
    memory_bfree(ADDR_OF(&counter->mctx), ADDR_OF(&counter->values), counter->range_size * sizeof(*counter->values));
}
...
interval_counter_advance_time(struct interval_counter *counter, uint32_t to) {
    ...
    while (counter->now < to) {
        uint64_t prev = values[counter->now & (counter->range_size - 1)].value; // because power of 2
        ++counter->now;
