 ...
 *
 * Each block stores exactly 16 uint32_t values (64 bytes total) and is
 ...
 *
 * The tree uses an implicit (b+1)-ary structure where b = 16.
 * Each node contains up to 16 values, and navigation is done through
...
btree_u32_nblocks(const struct btree_u32 *btree) {
	return btree->array.size / sizeof(struct btree_u32_block);
}

/**
 * @brief Calculate next node index in tree traversal
 * @param v Current node index
 * @param i Position within current block (0 to BTREE_U32_BLOCK_SIZE)
 * @return Index of child node
 *
 * For a (b+1)-ary tree where b = 16, the child at position i
...
btree_u32_build(
	...
	for (size_t i = 0; i < BTREE_U32_BLOCK_SIZE; ++i) {
		...
		size_t next = btree_u32_next(v, i);
		btree_u32_build(btree, next, data, idx, n, h + 1);

		// Fill current position if data remains
		if ((*idx) < n) {
			struct btree_u32_block *block =
				(struct btree_u32_block *)big_array_get(
					&btree->array,
	...
	size_t next = btree_u32_next(v, BTREE_U32_BLOCK_SIZE);
	btree_u32_build(btree, next, data, idx, n, h + 1);
}
...
btree_u32_init(
	...
	size_t nblocks = (n + BTREE_U32_BLOCK_SIZE - 1) / BTREE_U32_BLOCK_SIZE;
	size_t bytes = nblocks * sizeof(struct btree_u32_block);
	...
	size_t idx = 0;
	btree_u32_build(btree, 0, data, &idx, n, 0);
 ...
 * cannot be used until re-initialized with btree_u32_init().
 *
 * Example:
 * @code
 * btree_u32_fini(&tree);
 ...
 */
static inline void
btree_u32_fini(struct btree_u32 *btree) {
 ...
 *
 * idx = btree_u32_lower_bound(&tree, 10);
 * // Returns 2 (index of 10, exact match)
 *
 * idx = btree_u32_lower_bound(&tree, 25);
