...
__extension__({ \
    ...
    __TTLMAP_BUCKET_LOOKUP(__b, (key_ptr), (value_ptr), (now), __idx); \
})
...
__extension__({ \
    for (size_t i = 0; i < __TTLMAP_MAX_CHUNKS; ++i) { \
        if ((map_ptr)->chunks[i] != NULL) { \
            memory_bfree(&(map_ptr)->mctx, ADDR_OF(&(map_ptr)->chunks[i]), (map_ptr)->chunk_sizes[i]); \
        } \
    } \
})
...
__extension__({ \
    ...
    __res = __ttlmap_init_internal(map_ptr, mctx_ptr, alignof(__bucket_t), sizeof(__bucket_t), __bucket_count); \
    if (__res < 0) { \
        goto __done; \
    } \
...
__extension__({ \
    ...
    fprintf(file, "\tMemory used: %lu bytes\n", (map_ptr)->mctx.balloc_size); \
    fprintf(file, "\tKey-Value pairs per Bucket: %u\n", __TTLMAP_BUCKET_ENTRIES); \
    fprintf(file, "\tNumber of Buckets: %llu\n", (1ull << (map_ptr)->buckets_exp)); \
    fprintf(file, "\tPer Bucker memory overhead: %.2lf%%\n", 100.0 * (double)bucket_size / (__TTLMAP_BUCKET_ENTRIES * (sizeof(key_type) + sizeof(value_type)))); \
    fprintf(file, "\tAdditional Buckets memory overhead: %.2lf%%\n", 100.0 * (double)(map_ptr)->mctx.balloc_size / (bucket_size * (1ull << (map_ptr)->buckets_exp))); \
    fprintf(file, "\tNumber of Buckers per Chunk: %llu\n", (1ull << (map_ptr)->buckets_per_chunk_exp)); \
    size_t touched_counts[1 + __TTLMAP_BUCKET_ENTRIES]; \
    memset(touched_counts, 0, sizeof(touched_counts)); \
    for (size_t i = 0; i < (1ull << (map_ptr)->buckets_exp); ++i) { \
        size_t elems = __TTLMAP_BUCKET_ELEMENTS_TOUCHED(map_ptr, i, key_type, value_type); \
        ++touched_counts[elems]; \
    } \
    fprintf(file, "\tNumber of Buckers per Number of touched elements (0-%u): [", __TTLMAP_BUCKET_ENTRIES); \
    ...
    for (size_t i = 0; i < __TTLMAP_MAX_CHUNKS; ++i) { \
        if (i == 0 || i + 1 == __TTLMAP_MAX_CHUNKS || (map_ptr)->chunk_sizes[i] != (map_ptr)->chunk_sizes[i - 1]) { \
