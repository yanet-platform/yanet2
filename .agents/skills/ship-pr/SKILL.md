...
- **Discover GitHub MCP before choosing a client.** Lazy GitHub MCP tools may be absent from the initial tool list, so actively search the available tool metadata first — Claude Code exposes them under an `mcp__github__*` prefix, Codex under its own server name. If found, call the server's `get_me` before any other GitHub operation; use GitHub MCP for PR list/search/create/read/update, changed files, reviews, review threads/comments, check runs/status, replies, review actions, and merge.
...
2. Create the branch off confirmed `origin/main`. Use a **shell-safe** name — `<type>/<scope>-<what>` (e.g. `chore/ai-ship-pr`): no `(`/`)` (bash mis-parses them unquoted in git commands), and avoid a name that is also a path prefix of another branch (see stacked-PR ref):
   - Worktree (the default, and mandatory unless the user waived isolation for this task): `git worktree add <worktrees>/<name> -b <branch> origin/main`, where `<worktrees>` is the client's root-local gitignored worktree directory (`.claude/worktrees/` for Claude Code, `.agent-state/worktrees/` for Codex — see AGENTS.md under "Worktree isolation"), never a sibling of the repository. Put a task that needs its own full C/DPDK build on a volume with room for it instead.
...
5. After pushing a fix, reply to and resolve the addressed thread through MCP when supported; otherwise use `gh api graphql` and record why. Reply with `add_reply_to_pull_request_comment`, whose `commentId` is the numeric ID parsed from the comment's `html_url` `#discussion_r...` anchor, NOT the `threadId` from `get_review_comments` (a GraphQL thread node ID, which the tool rejects) — but a finding that arrived as a plain issue comment has neither an inline-comment ID nor a review thread, so reply to it with an ordinary PR comment through `add_issue_comment` instead and skip resolution for it, since there is no thread to resolve. Main's ruleset requires thread resolution for a finding that has one: resolve with `pull_request_review_write` method `resolve_thread` (its `threadId` comes from `get_review_comments`). Re-summon with a `@codex review` comment through `add_issue_comment` because force-push alone does not retrigger review.
6. Return to step 1 for the new head and reread all three: PR-level reviews, inline threads, and issue comments. After at least one addressed round with green checks, step 3's completion gate satisfied for the new head, and no unresolved findings, proceed to Phase 6 only when merge was authorized — this list is not the whole merge precondition, Phase 6's own gate still applies; otherwise leave the PR ready to merge and report it.

### Phase 6 — Merge (only if "merge" was authorized)

- **Precondition — the `needs-architect` gate** (see non-negotiables): collect every issue this PR names anywhere in its body — a `#<n>` under any keyword or none, `Closes`, `Part of`, `Addresses part of`, a bare mention — plus the tracked issue the work was started under whether or not the body names it, then read each one's labels with `issue_read` method `get_labels` against the repo the PR lives in, since the label exists in both `yanet2` and `yanet2-private`. A failed label read is ambiguous, since `get_labels` returns the same "Could not resolve to an Issue" error for a number that is a pull request and for one that does not exist, so settle it with `issue_read` method `get` on that number: a success whose URL points at a pull request is not a hit, and anything else, a 404 included, is a block rather than a pass. Blocking on a merely mentioned issue is the intended conservative default, since one line from the user lifts it. Unlifted hit → do not merge, report it and stop. The block clears on its own if the label is gone from the issue by the time you read it.
- Merge through GitHub MCP. **Single-commit PR** → `merge_pull_request` with `merge_method: squash`. **Multi-commit, deliberately structured history** → `merge_method: rebase` (preserve it). **Multi-commit where extras are review fixups** → `merge_method: squash` with an explicit clean `commit_title: "<type>(<scope>): <title> (#N)"` and `commit_message: "<high-level or empty>"`. Never let GitHub's default squash body (a bullet list of every intermediate commit) land — the user calls that "каша" and it is a convention violation.
...
1. `git worktree remove <worktrees>/<name>` (if used).
...
4. `git push origin --delete`'s own output is not evidence the ref is gone: it can print "Everything up-to-date" and exit clean while the remote ref survives untouched at its old SHA, with the same form deleting normally on another branch minutes earlier in the same session — this is why deletion here is a verified lease, not a bare delete command. Before remote deletion, query the exact remote ref. If it is absent, skip deletion. If it differs from `$recorded_pr_head_sha`, stop and report it. Use the expected-old-SHA lease for deletion, so a race that advances the remote ref fails safely:
...
Report to the user: PR number(s) and state (open / merged), any review findings addressed, any flakes you re-ran, every `gh` fallback with its reason, any `needs-architect` block you hit (the PR, the issue and whether the user lifted it), and (if "create a PR" not "merge") that CI is green and it's ready to merge on their word.
