...
- **Precondition — the `needs-architect` gate** (see non-negotiables): collect every issue this PR names anywhere in its body — a `#<n>` under any keyword or none, `Closes`, `Part of`, `Addresses part of`, a bare mention — plus the tracked issue the work was started under whether or not the body names it, then read each one's labels through GitHub MCP against the repo the PR lives in, since the label exists in both `yanet2` and `yanet2-private`. A failed label read is ambiguous, since the same "could not resolve to an issue" error covers a number that is a pull request and one that does not exist, so settle it by reading that number as an issue through GitHub MCP: a success whose URL points at a pull request is not a hit, and anything else, a 404 included, is a block rather than a pass. Blocking on a merely mentioned issue is the intended conservative default, since one line from the user lifts it. Unlifted hit → do not merge, report it and stop. The block clears on its own if the label is gone from the issue by the time you read it.
