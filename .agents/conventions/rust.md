...
- **Doc comments**: `///`/`//!` are one line, only where the item is not self-explanatory; the same applies to a plain `//` comment. `.rustfmt.toml`'s `wrap_comments` reflows any of these past `comment_width` (80 columns) into multiple lines on `cargo +nightly fmt`, so keep a line within that width — the formatter's wrap is not an exemption from the one-line ceiling.
