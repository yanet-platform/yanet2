...
- **Comments**: one line, only where the code is not self-explanatory; `.clang-format`'s default `ReflowComments` wraps anything past `ColumnLimit` (80 columns) on `clang-format -i`, so keep a line within that width.
