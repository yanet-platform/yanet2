...
- **Tests**: follow `.agents/conventions/tests.md` (naming `Test_<WhatToTest>_<Case>`, one-brief `verifies that` doc comment, self-describing table-case `name:`); table-driven with `require.NoError(t, err)`. Reference: `modules/blackhole/controlplane/service_test.go`. Production comments never mention tests. `_test.go` uses `package <pkg>_test` and `testpkg` gates it; `package main` is exempt because it is unimportable.
