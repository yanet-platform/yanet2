...
- **Comments**: follow `.agents/conventions/comments.md` (brief + blank + detailed); English, period-terminated, only where the code is not self-explanatory; list production callers only; an exported doc comment opens with the symbol's own name and keeps the blank-line synopsis/body split that `go doc` relies on. `gofmt` does not wrap, so keep each line within ~80 columns by hand. The C preamble of a `import "C"` file (`#cgo`, `#include`, C prototypes, in-preamble C comments) is exempt; Go-side `//` comments there follow the rule as normal.
...
- **Tests**: follow `.agents/conventions/tests.md` (naming `Test_<WhatToTest>_<Case>`, a doc comment opening `// Test_<WhatToTest>_<Case> verifies that …`, self-describing table-case `name:`); table-driven with `require.NoError(t, err)`. Reference: `modules/blackhole/controlplane/service_test.go`. Production comments never mention tests. `_test.go` uses `package <pkg>_test` and `testpkg` gates it; `package main` is exempt because it is unimportable.
