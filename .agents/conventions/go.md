...
- **Comments**: English, period-terminated, one unwrapped line even past ~80 columns, only where the code is not self-explanatory; list production callers only; an exported doc comment opens with the symbol's own name.
