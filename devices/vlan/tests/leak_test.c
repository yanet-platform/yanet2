...
main(void) {
	...
	TEST_ASSERT_EQUAL(
		res, 0, "cp_device_vlan_config_set_input_pipeline failed"
	);

	res = cp_device_vlan_config_set_output_pipeline(cfg, 0, "p0", 1);
	TEST_ASSERT_EQUAL(
		res, 0, "cp_device_vlan_config_set_output_pipeline failed"
	);

	cp_device_vlan_config_free(cfg);
