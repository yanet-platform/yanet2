...
func NewFWStateModule(cfg *Config, options ...Option) (*FWStateModule, error) {
	...
	fwstateService := NewFWStateService(
		...
		// Reads delegated to linked map objects resolve through the map
		// service's internal registry instead of the C API registry
		// lookup.
		WithMapLookup(mapService.LookupMap),
	)
	fwstateMetricsService := NewMetricsService(fwstateService)
