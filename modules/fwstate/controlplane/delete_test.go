...
// lookup resolves one pair member by name and family, mirroring the
// objects controlplane's internal-registry lookup.
func (m fwstateTestMaps) lookup(name string, kind objfwstate.Kind) *objfwstate.MapObjectConfig {
	var object *objfwstate.MapObjectConfig
	switch kind {
	case objfwstate.KindV4:
		object = m.v4
	case objfwstate.KindV6:
		object = m.v6
	default:
		return nil
	}
...
// mapLookupFor combines test map pairs into the resolver the module
// service reads linked map objects through.
func mapLookupFor(pairs ...fwstateTestMaps) fwstate.MapLookup {
	return func(name string, kind objfwstate.Kind) *objfwstate.MapObjectConfig {
		for idx := range pairs {
			if object := pairs[idx].lookup(name, kind); object != nil {
				return object
			}
		}
		return nil
...
// C-side error naming the object, and that nothing is published by the
// failed update.
