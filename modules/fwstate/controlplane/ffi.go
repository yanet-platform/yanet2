...
// for a fresh config) and the named map objects are linked.
//
...
// Generation returns the counter incremented on every published update of
// this config, for clients detecting a config change mid-iteration.
func (m *FwStateConfig) Generation() uint64 {
	return m.generation
}

// MarkPublished advances the generation counter after the config has been
// published to the dataplane.
func (m *FwStateConfig) MarkPublished() {
	m.generation++
}
