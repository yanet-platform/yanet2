...
#include "modules/mirror/dataplane/dataplane.h"

#include "lib/fuzzing/fuzzing.h"

// Mirror module filter compilation needs more memory than the default 1 MB
// arena.
//
// ASAN redzones inflate allocations significantly, so 16 MB is needed for
// fuzzing builds.
#define MIRROR_EXTRA_ARENA_SIZE (16 << 20)

static struct fuzzing_params fuzz_params = {0};

static int
mirror_test_config(struct cp_module **cp_module, yanet_error **err) {
	struct mirror_module_config *config =
		(struct mirror_module_config *)memory_balloc(
			&fuzz_params.mctx, sizeof(struct mirror_module_config)
		);
	...
	memset(config, 0, sizeof(struct mirror_module_config));
	...
	//
	// Needed by "mirror_module_config_update".
	...
	int rc = mirror_module_config_update(&config->cp_module, rules, 3, err);
	if (rc != 0) {
		goto fail;
	}

	// Set up counter storage, because "mirror_handle_packets" accesses
	// counters.
...
fuzz_setup(yanet_error **err) {
	if (fuzzing_params_init(
		    &fuzz_params, "mirror fuzzing", new_module_mirror
	    ) != 0) {
		return EXIT_FAILURE;
	}

	// Add extra memory arena for filter compilation.
	void *arena = malloc(MIRROR_EXTRA_ARENA_SIZE);
	if (arena == NULL) {
		return EXIT_FAILURE;
	}
	block_allocator_put_arena(
		&fuzz_params.ba, arena, MIRROR_EXTRA_ARENA_SIZE
	);

	// Create a minimal "dp_worker" for counter access and packet cloning.
	fuzz_params.worker =
	...
	// The mirror handler clones every packet from this mempool.
	fuzz_params.worker->rx_mempool = fuzz_params.mempool;

	return mirror_test_config(&fuzz_params.cp_module, err);
}
