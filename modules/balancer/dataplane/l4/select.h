...
next_rnd(struct virtual_service *vs, struct packet_metadata *meta, size_t worker) {
	return vs->flags & BALANCER_VS_PRR_FLAG ? vs->worker_local[worker].round_robin_counter++
						: meta->hash;
}
...
select_real(
	...
	if (vs->flags & BALANCER_VS_OPS_FLAG) {
		uint32_t real_id =
			ring_get(&vs->real_ring, next_rnd(vs, metadata, ctx->worker->idx));
	...
	uint32_t real_id = ring_get(&vs->real_ring, next_rnd(vs, metadata, ctx->worker->idx));
	if (real_id == RING_VALUE_INVALID) {
		VS_STATS_INC(no_reals, ctx);
		session_remove(session_state); // free created state
