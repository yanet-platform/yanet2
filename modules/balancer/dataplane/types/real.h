...
enum balancer_real_flags {
	...
	balancer_real_removed = 1u << 1,
	balancer_real_ipv6 = 1u << 2,
};
...
struct balancer_real {
	...
	/*
	 * Encodes (epoch << 32) | config_index.
	 * Sessions store this value; on reuse the dataplane compares
	 * the stored value against the current one to detect
	 * replacements without any map lookup.
	 */
	uint64_t stable_idx;
