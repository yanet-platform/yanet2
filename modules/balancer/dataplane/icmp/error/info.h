...
fill_icmp_packet_info_ipv4(
	...
	const struct rte_ipv4_hdr *ipv4_hdr = rte_pktmbuf_mtod_offset(
		mbuf, struct rte_ipv4_hdr *, info->inner.network.offset
	);

	/// @todo: check version

	if (rte_pktmbuf_pkt_len(mbuf) <
	    (uint32_t)info->inner.network.offset +
		    rte_be_to_cpu_16(ipv4_hdr->total_length)) {
		info->inner.network.type = PACKET_INFO_UNKNOWN;
		return -1;
	}
	...
	if ((ipv4_hdr->version_ihl & 0x0F) < 0x05) {
		...
		info->inner.transport.type = ipv4_hdr->next_proto_id;
		info->inner.transport.offset =
...
fill_icmp_packet_info_ipv6(
	...
	const struct rte_ipv6_hdr *ipv6_hdr = rte_pktmbuf_mtod_offset(
		mbuf, struct rte_ipv6_hdr *, info->inner.network.offset
	);

	/// @todo: check version

	if (rte_pktmbuf_pkt_len(mbuf) <
	    (uint32_t)info->inner.network.offset + sizeof(struct rte_ipv6_hdr) +
	...
	uint16_t transport_hdr_offset =
		info->inner.network.offset + sizeof(struct rte_ipv6_hdr);
	...
	for (extension_i = 0; extension_i < PACKET_INFO_EXTENSIONS_MAX + 1;
		...
		if (transport_hdr_type == IPPROTO_HOPOPTS ||
			...
			/** @todo: last extension?
			info->inner.transport.type = transport_headerType;
			info->inner.transport.offset = transport_headerOffset;

			break;
			*/
		} else {
			info->inner.transport.type = transport_hdr_type;
			info->inner.transport.offset = transport_hdr_offset;
	...
	if (rte_be_to_cpu_16(ipv6_hdr->payload_len) <
	    info->inner.transport.offset - info->inner.network.offset -
...
fill_icmp_packet_info(struct rte_mbuf *mbuf, struct icmp_packet_info *info) {
	if (info->inner.network.type == rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4)) {
		return fill_icmp_packet_info_ipv4(mbuf, info);
	} else if (info->inner.network.type ==
