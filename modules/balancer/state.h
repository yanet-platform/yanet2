...
struct balancer_sessions_storage_gen {
    ...
    __rte_cache_aligned struct worker_local worker_local[BALANCER_MAX_WORKERS_NUM];
};
...
static inline int
balancer_get_session(struct balancer_state *state, size_t worker_idx, uint32_t now, uint32_t timeout, struct balancer_session_id *session_id, struct balancer_session_state **session_state, balancer_session_lock_t **lock) {
    struct balancer_sessions_storage_gen *sessions_cur = balancer_get_cur_storage_gen(state);
    int ret = TTLMAP_GET(&sessions_cur->session_table, session_id, session_state, lock, now, timeout);
    struct worker_local *worker_local = &sessions_cur->worker_local[worker_idx];
    if (ret == TTLMAP_FOUND) {
        ...
        if (worker_local->use_prev_gen == 1) {
            if (worker_local->max_deadline_prev_gen + STATE_TIMEOUT_DEFAULT < now) {
            ...
            ret = TTLMAP_LOOKUP(&sessions_prev->session_table, session_id, *session_state, now);
            if (ret == TTLMAP_FOUND) {
