...
func smallConfig() (*balancerpb.ModuleConfig, *balancerpb.SessionsTimeouts) {
	config := &balancerpb.ModuleConfig{
		...
		VirtualServices: []*balancerpb.VirtualService{
			{
				Addr: balancer.IpAddr("192.166.13.22").AsSlice(),
				Port: 1000,
				...
				AllowedSrcs: []*balancerpb.Subnet{
					{
						Addr: balancer.IpAddr("10.12.0.0").AsSlice(),
						Size: 8,
					},
				},
				Reals: []*balancerpb.Real{
					{
						Weight:  1,
						DstAddr: balancer.IpAddr("1.1.1.1").AsSlice(),
						SrcAddr: balancer.IpAddr("3.3.3.3").AsSlice(),
						SrcMask: balancer.IpAddr("255.240.255.0").AsSlice(),
					},
					{
						Weight:  2,
						DstAddr: balancer.IpAddr("2.2.2.2").AsSlice(),
						SrcAddr: balancer.IpAddr("3.3.3.3").AsSlice(),
						SrcMask: balancer.IpAddr("255.240.255.0").AsSlice(),
					},
					{
						Weight:  2,
						DstAddr: balancer.IpAddr("3.3.3.3").AsSlice(),
						SrcAddr: balancer.IpAddr("3.3.3.3").AsSlice(),
						SrcMask: balancer.IpAddr("255.240.255.0").AsSlice(),
					},
				},
...
func smallSetup(t *testing.T) *balancer.TestSetup {
...
func allowedSrc(idx uint8) netip.Addr {
	return balancer.IpAddr(fmt.Sprintf("10.12.0.%d", idx))
}
...
func TestSelectAfterUpdate(t *testing.T) {
	...
	t.Run("Send_Some_Packets_Before_Update", func(t *testing.T) {
		...
		info := bal.GetStateInfo(mock.CurrentTime())
		...
		// validate state info
		balancer.ValidateStateInfo(t, info, balancer.GetModuleConfig().VirtualServices)
	})
	...
	t.Run("Send_Some_Packets_After_Second_Update", func(t *testing.T) {
		// set prev state info
		infoBefore := bal.GetStateInfo(mock.CurrentTime())
...
func TestNewConfig(t *testing.T) {
	...
	vsIp := balancer.IpAddr("192.160.11.1")
	vsPort := uint16(1015)
	vsProto := balancerpb.TransportProto_TCP
	vsAllowedSrc := []*balancerpb.Subnet{
		{
			Addr: balancer.IpAddr("10.0.1.0").AsSlice(),
			Size: 24,
		},
	}
		...
		VirtualServices: []*balancerpb.VirtualService{
			{
				...
				Reals: []*balancerpb.Real{
					{
						DstAddr: balancer.IpAddr("10.1.1.1").AsSlice(),
						Weight:  1,
						SrcAddr: balancer.IpAddr("1.1.1.1").AsSlice(),
						SrcMask: balancer.IpAddr("1.1.1.1").AsSlice(),
					},
				},
			},
		},
	...
	// update config
	_, stateConfig := bal.GetConfig()
	err := bal.Update(config, stateConfig)
	require.NoError(t, err)

	// send packet, it is scheduled on the first real

	clientIp := balancer.IpAddr("10.0.1.22")
	...
	t.Run("Send_First_Packet", func(t *testing.T) {
		...
		balancer.ValidatePacket(t, balancer.GetModuleConfig(), packet, result.Output[0])
	})
		...
		VirtualServices: []*balancerpb.VirtualService{
			{
				...
				Reals: []*balancerpb.Real{
					{
						DstAddr: balancer.IpAddr("10.12.2.2").AsSlice(),
						Weight:  1,
						SrcAddr: balancer.IpAddr("133.12.13.11").AsSlice(),
						SrcMask: balancer.IpAddr("255.0.240.192").AsSlice(),
					},
				},
			},
		},
