...
func TestSessionTableStress2(t *testing.T) {
	...
	virtualServicesConfig := []vsConfig{
		// VS1: TCP IPv4, PRR, no GRE, no FixMSS, IPv4 reals
		{
			...
			fixMss:    false,
			reals: []realConfig{
				{balancer.IpAddr("10.2.1.1"), 1},
				{balancer.IpAddr("10.2.1.2"), 1},
			},
		},
		// VS2: UDP IPv4, WLC, no GRE, IPv4 reals
		{
			ip:        balancer.IpAddr("10.1.2.1"),
			port:      5353,
			...
			fixMss:    false,
			reals: []realConfig{
				{balancer.IpAddr("10.2.2.1"), 2},
				{balancer.IpAddr("10.2.2.2"), 3},
			},
		},
		// VS3: TCP IPv6, PRR, with GRE, IPv6 reals
		{
			ip:        balancer.IpAddr("2001:db8::1"),
			port:      443,
			proto:     balancerpb.TransportProto_TCP,
			scheduler: balancerpb.VsScheduler_PRR,
			gre:       true,
			fixMss:    false,
			reals: []realConfig{
				{balancer.IpAddr("2001:db8:2::1"), 1},
				{balancer.IpAddr("2001:db8:2::2"), 1},
			},
		},
		// VS4: UDP IPv6, WLC, IPv6 reals
		{
			ip:        balancer.IpAddr("2001:db8::2"),
			port:      8080,
			proto:     balancerpb.TransportProto_UDP,
			scheduler: balancerpb.VsScheduler_WLC,
			gre:       false,
			fixMss:    false,
			reals: []realConfig{
				{balancer.IpAddr("2001:db8:3::1"), 1},
				{balancer.IpAddr("2001:db8:3::2"), 2},
			},
		},
		// VS5: TCP IPv4, WLC, with GRE, mixed IPv4 and IPv6 reals
		{
			ip:        balancer.IpAddr("10.1.3.1"),
			port:      8443,
			proto:     balancerpb.TransportProto_TCP,
			scheduler: balancerpb.VsScheduler_WLC,
			gre:       true,
			fixMss:    false,
			reals: []realConfig{
				{balancer.IpAddr("10.2.4.1"), 3},
				{balancer.IpAddr("2001:db8:4::2"), 2},
			},
		},
		// VS6: TCP IPv6, PRR, with FixMSS, IPv4 reals
		{
			ip:        balancer.IpAddr("2001:db8::3"),
			port:      9000,
			proto:     balancerpb.TransportProto_TCP,
			scheduler: balancerpb.VsScheduler_PRR,
			gre:       false,
			fixMss:    true,
			reals: []realConfig{
				{balancer.IpAddr("10.2.3.1"), 1},
				{balancer.IpAddr("10.2.3.2"), 1},
			},
		},
	}
	...
	for _, vsConf := range virtualServicesConfig {
		...
		for _, realConf := range vsConf.reals {
			...
			if realConf.ip.Is4() {
				...
				srcMask = balancer.IpAddr("ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff").AsSlice()
			}
	...
	makeTcpSynPacket := func(clientIP netip.Addr, clientPort uint16, vsIP netip.Addr, vsPort uint16) gopacket.Packet {
		...
		p, err := balancer.InsertOrUpdateMSS(packet, 1460)
	...
	t.Run("Phase1_Create_16_Initial_Sessions", func(t *testing.T) {
		...
		for i := range 16 {
			...
			if vs.proto == balancerpb.TransportProto_TCP {
				...
				packetLayers := balancer.MakeUDPPacket(
					clientIP,
					clientPort,
					vs.ip,
					vs.port,
		...
		for i, outPacket := range result.Output {
			...
			balancer.ValidatePacket(t, balancer.GetModuleConfig(), packets[i], outPacket)
	...
	t.Run("Final_Verification", func(t *testing.T) {
		...
		// Get state info
		stateInfo := bal.GetStateInfo(mock.CurrentTime())
