...
func TestHashConsistency(t *testing.T) {
	// Common test data
	vs1Ip := test_utils.IpAddr("1.1.1.1")
	vs1Port := uint16(80)
	vs2Ip := test_utils.IpAddr("1.1.1.2")
	vs2Port := uint16(8080)
	real1Ip := test_utils.IpAddr("2.2.2.2")
	real2Ip := test_utils.IpAddr("3.3.3.3")
	clientIp := test_utils.IpAddr("10.0.0.100")
	...
	t.Run("Expired_Sessions_Hash_Consistency", func(t *testing.T) {
		...
		for round := range numRounds {
			...
			err = test_utils.SyncActiveSessionsAndWlcAndResizeTableOnDemand(
				mock.CurrentTime(),
			)
