...
static int
test_ring_basic_usage(struct memory_context *memory_context) {
	struct ring ring;
	ring_init(&ring, memory_context, 3);

	ring_change_weight(&ring, 0, 1);
	ring_change_weight(&ring, 1, 1);
	ring_change_weight(&ring, 2, 1);

	// Just verify we can get values and they're in expected range
	for (int i = 0; i < 100; i++) {
		uint32_t id = ring_get(&ring, i);
		TEST_ASSERT(id < 3, "ring_get returns valid real id");
	}
