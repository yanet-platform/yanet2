...
func Encap(t *testing.T, origLayers []gopacket.SerializableLayer, srcIP string, dstIP string) gopacket.Packet {
...
func MakeUDPPacket(srcIP string, srcPort uint16, dstIP string, dstPort uint16) []gopacket.SerializableLayer {
	...
	layers := []gopacket.SerializableLayer{eth, ip.(gopacket.SerializableLayer), udp, gopacket.Payload(payload)}

	return layers
}

func MakeTCPPacket(srcIP string, srcPort uint16, dstIP string, dstPort uint16, tcp *layers.TCP) []gopacket.SerializableLayer {
	...
	payload := []byte("PING TEST PAYLOAD 1234567890")
	layers := []gopacket.SerializableLayer{eth, ip.(gopacket.SerializableLayer), tcp, gopacket.Payload(payload)}
