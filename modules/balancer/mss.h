...
__csum_plus(uint16_t val0, uint16_t val1) { // NOLINT
...
static inline uint16_t
__csum_minus(uint16_t val0, uint16_t val1) { // NOLINT
	uint16_t sum = val0 - val1;
	...
	if (packet->transport_header.type == IPPROTO_TCP) {
		...
		while (tcp_option_offset + TCP_OPTION_MSS_LEN <= tcp_data_offset
			...
			if (option->kind == TCP_OPTION_KIND_MSS) {
				...
				*(uint16_t *)option->data = rte_cpu_to_be_16(
					YANET_BALANCER_FIX_MSS_SIZE
				);
				cksum = __csum_plus(
					cksum, *(uint16_t *)option->data
				);
		...
		*(uint16_t *)option->data =
			rte_cpu_to_be_16(YANET_BALANCER_DEFAULT_MSS_SIZE);
