...
func FillBalancerStatsC(
	...
	stats := (*C.struct_balancer_stats_info)(
		C.calloc(1, C.size_t(unsafe.Sizeof(*(*C.struct_balancer_stats_info)(nil)))),
	)
	if stats == nil {
	...
	if int(rc) != 0 {
		C.free(unsafe.Pointer(stats))
		return nil, fmt.Errorf("balancer_stats_info_fill returned non-zero: %d", int(rc))
	}
...
func BalancerConfigStats(
	...
	handle, err := FillBalancerStatsC(agent, device, pipeline, function, chain, moduleName)
	if err != nil {
		return nil, err
	}
	defer handle.free()
...
func copyFromC(state ModuleConfigStatePtr, cstats *C.struct_balancer_stats_info) *module.BalancerStats {
	...
	out.Module.ICMPv4 = module.ICMPStats{
		IncomingPackets:           uint64(cstats.stats.icmp_ipv4.incoming_packets),
		EchoResponses:             uint64(cstats.stats.icmp_ipv4.echo_responses),
		PayloadTooShortIP:         uint64(cstats.stats.icmp_ipv4.payload_too_short_ip),
		UnmatchingSrcFromOriginal: uint64(cstats.stats.icmp_ipv4.unmatching_src_from_original),
		PayloadTooShortPort:       uint64(cstats.stats.icmp_ipv4.payload_too_short_port),
		UnexpectedTransport:       uint64(cstats.stats.icmp_ipv4.unexpected_transport),
		UnrecognizedVS:            uint64(cstats.stats.icmp_ipv4.unrecognized_vs),
		ForwardedPackets:          uint64(cstats.stats.icmp_ipv4.forwarded_packets),
		BroadcastedPackets:        uint64(cstats.stats.icmp_ipv4.broadcasted_packets),
		PacketClones:              uint64(cstats.stats.icmp_ipv4.packet_clones),
		PacketCloneFailures:       uint64(cstats.stats.icmp_ipv4.packet_clone_failures),
	}

	// ICMP IPv6
	out.Module.ICMPv6 = module.ICMPStats{
		IncomingPackets:           uint64(cstats.stats.icmp_ipv6.incoming_packets),
		EchoResponses:             uint64(cstats.stats.icmp_ipv6.echo_responses),
		PayloadTooShortIP:         uint64(cstats.stats.icmp_ipv6.payload_too_short_ip),
		UnmatchingSrcFromOriginal: uint64(cstats.stats.icmp_ipv6.unmatching_src_from_original),
		PayloadTooShortPort:       uint64(cstats.stats.icmp_ipv6.payload_too_short_port),
		UnexpectedTransport:       uint64(cstats.stats.icmp_ipv6.unexpected_transport),
		UnrecognizedVS:            uint64(cstats.stats.icmp_ipv6.unrecognized_vs),
		ForwardedPackets:          uint64(cstats.stats.icmp_ipv6.forwarded_packets),
		BroadcastedPackets:        uint64(cstats.stats.icmp_ipv6.broadcasted_packets),
		PacketClones:              uint64(cstats.stats.icmp_ipv6.packet_clones),
		PacketCloneFailures:       uint64(cstats.stats.icmp_ipv6.packet_clone_failures),
	}
	...
	out.Module.Common = module.CommonStats{
		...
		UnexpectedNetworkProto: uint64(cstats.stats.common.unexpected_network_proto),
	...
	if vsCount > 0 && cstats.vs_info != nil {
		cArr := unsafe.Slice((*C.struct_balancer_vs_stats_info)(cstats.vs_info), vsCount)
		...
		for i := range vsCount {
			...
			stats := module.VsStats{
				...
				PacketSrcNotAllowed:    uint64(entry.stats.packet_src_not_allowed),
				NoReals:                uint64(entry.stats.no_reals),
				OpsPackets:             uint64(entry.stats.ops_packets),
				SessionTableOverflow:   uint64(entry.stats.session_table_overflow),
				...
				NotRescheduledPackets:  uint64(entry.stats.not_rescheduled_packets),
				BroadcastedIcmpPackets: uint64(entry.stats.broadcasted_icmp_packets),
	...
	if realCount > 0 && cstats.real_info != nil {
		cArr := unsafe.Slice((*C.struct_balancer_real_stats_info)(cstats.real_info), realCount)
		...
		for i := range realCount {
			...
			stats := module.RealStats{
				PacketsRealDisabled:   uint64(entry.stats.packets_real_disabled),
				PacketsRealNotPresent: uint64(entry.stats.packets_real_not_present),
				OpsPackets:            uint64(entry.stats.ops_packets),
				ErrorIcmpPackets:      uint64(entry.stats.error_icmp_packets),
				CreatedSessions:       uint64(entry.stats.created_sessions),
