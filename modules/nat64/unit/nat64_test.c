...
print_upkt(struct upkt *pkt) {
	...
	if (pkt->eth.ether_type == RTE_BE16(RTE_ETHER_TYPE_IPV4)) {
		...
		RTE_LOG(INFO, NAT64_TEST, "  Fragment Offset: %d\n",
		       rte_be_to_cpu_16(pkt->ip.ipv4.fragment_offset) & RTE_IPV4_HDR_OFFSET_MASK);
...
print_rte_mbuf(struct rte_mbuf *mbuf) {
	...
	if (eth_hdr->ether_type == RTE_BE16(RTE_ETHER_TYPE_IPV4)) {
		...
		RTE_LOG(INFO, NAT64_TEST, "  Fragment Offset: %d\n",
		       rte_be_to_cpu_16(ipv4_hdr->fragment_offset) & RTE_IPV4_HDR_OFFSET_MASK);
...
static inline int compare_ethernet_headers(struct rte_ether_hdr *eth_hdr, struct upkt *upkt) {
    int result = 0;

    if (memcmp(eth_hdr->dst_addr.addr_bytes, upkt->eth.dst_addr.addr_bytes, 6) != 0) {
        result = -1;
        RTE_LOG(ERR, NAT64_TEST, "Difference in Ethernet destination address\n");
        RTE_LOG(ERR, NAT64_TEST, "UPKT: " RTE_ETHER_ADDR_PRT_FMT "\n", RTE_ETHER_ADDR_BYTES(&upkt->eth.dst_addr));
        RTE_LOG(ERR, NAT64_TEST, "MBUF:" RTE_ETHER_ADDR_PRT_FMT "\n", RTE_ETHER_ADDR_BYTES(&eth_hdr->dst_addr));
    }

    if (memcmp(eth_hdr->src_addr.addr_bytes, upkt->eth.src_addr.addr_bytes, 6) != 0) {
        result = -1;
...
static inline int compare_ipv4_headers(struct rte_ipv4_hdr *ipv4_hdr, struct upkt *upkt) {
    int result = 0;
    ...
    if (ipv4_hdr->src_addr != upkt->ip.ipv4.src_addr) {
        ...
        RTE_LOG(ERR, NAT64_TEST, "UPKT: "IPv4_BYTES_FMT"\n", IPv4_BYTES_LE(upkt->ip.ipv4.src_addr));
        RTE_LOG(ERR, NAT64_TEST, "MBUF: "IPv4_BYTES_FMT"\n", IPv4_BYTES_LE(ipv4_hdr->src_addr));
    }
...
static inline int compare_ipv6_headers(struct rte_ipv6_hdr *ipv6_hdr, struct upkt *upkt) {
    int result = 0;

    if ((htons(ipv6_hdr->vtc_flow & 0xF0000000)) >> 28 != (htons(upkt->ip.ipv6.vtc_flow) & 0xF0000000) >> 28) {
        result = -1;
    ...
    if (memcmp(&ipv6_hdr->src_addr, &upkt->ip.ipv6.src_addr, sizeof(struct in6_addr)) != 0) {
        result = -1;
        RTE_LOG(ERR, NAT64_TEST, "Difference in IPv6 source address\n");
        RTE_LOG(ERR, NAT64_TEST, "UPKT: "IPv6_BYTES_FMT"\n", IPv6_BYTES(upkt->ip.ipv6.src_addr));
        RTE_LOG(ERR, NAT64_TEST, "MBUF: "IPv6_BYTES_FMT"\n", IPv6_BYTES(ipv6_hdr->src_addr));
    }

    if (memcmp(&ipv6_hdr->dst_addr, &upkt->ip.ipv6.dst_addr, sizeof(struct in6_addr)) != 0) {
        result = -1;
...
static inline int compare_udp_headers(struct rte_udp_hdr *udp_hdr, struct upkt *upkt) {
    int result = 0;
...
static inline int compare_tcp_headers(struct rte_tcp_hdr *tcp_hdr, struct upkt *upkt) {
    int result = 0;
    ...
    if (tcp_hdr->recv_ack != upkt->proto.tcp.recv_ack) {
        result = -1;
        RTE_LOG(ERR, NAT64_TEST, "Difference in TCP acknowledgment number\n");
        RTE_LOG(ERR, NAT64_TEST, "UPKT: %u\n", ntohl(upkt->proto.tcp.recv_ack));
        RTE_LOG(ERR, NAT64_TEST, "MBUF: %u\n", ntohl(tcp_hdr->recv_ack));
    }

    if ((tcp_hdr->data_off & 0xF0) >> 4 != (upkt->proto.tcp.data_off & 0xF0) >> 4) {
        result = -1;
...
static inline int compare_icmp_headers(struct icmphdr *icmp_hdr, struct upkt *upkt) {
    int result = 0;
    ...
    if (icmp_hdr->un.gateway != upkt->proto.icmp.un.gateway) {
        ...
        RTE_LOG(ERR, NAT64_TEST, "UPKT: 0x%08X\n", htonl(upkt->proto.icmp.un.gateway));
        RTE_LOG(ERR, NAT64_TEST, "MBUF: 0x%08X\n", htonl(icmp_hdr->un.gateway));
    }
...
static inline int compare_icmp6_headers(struct icmp6_hdr *icmp6_hdr, struct upkt *upkt) {
    int result = 0;
...
print_diff_upkt_and_rte_mbuf(struct upkt *upkt, struct rte_mbuf *mbuf) {
    ...
    if (eth_hdr->ether_type == RTE_BE16(RTE_ETHER_TYPE_IPV4)) {
        struct rte_ipv4_hdr *ipv4_hdr = (struct rte_ipv4_hdr *)ip_hdr_offset;
        data_off += rte_ipv4_hdr_len(ipv4_hdr);
        ...
        switch (ipv4_hdr->next_proto_id) {
            case IPPROTO_UDP:
                data_off += sizeof(struct rte_udp_hdr);
                struct rte_udp_hdr *udp_hdr = (struct rte_udp_hdr *)proto_data;
        ...
        struct rte_ipv6_hdr *ipv6_hdr = (struct rte_ipv6_hdr *)ip_hdr_offset;
        data_off += sizeof(struct rte_ipv6_hdr);
    ...
    if ((mbuf->data_len - data_off) != upkt->data_len) {
        ...
        uint8_t *mbuf_data = rte_pktmbuf_mtod_offset(mbuf, uint8_t *, data_off);
        if (memcmp(mbuf_data, upkt->data, upkt->data_len) != 0) {
            ...
            for(int i = 0; i < upkt->data_len; i++) {
                ...
                RTE_LOG(ERR, NAT64_TEST, "%d: 0x%02x %s 0x%02x\n", i, u, u == mbuf_data[i] ? "=" : "!=", mbuf_data[i]);
            }
        }
    }
...
upkt_ipv4_updtcp_checksum(struct rte_ipv4_hdr *ipv4_hdr, void *l4_hdr, uint16_t l4_len, void* payload, uint16_t payload_len) {
	uint32_t cksum;
...
static inline uint16_t
upkt_ipv6_updtcp_checksum(struct rte_ipv6_hdr *ipv6_hdr, void *l4_hdr, uint16_t l4_len, void* payload, uint16_t payload_len) {
	uint32_t cksum;
...
fix_checksums(struct upkt *pkt) {
    if (pkt->eth.ether_type == RTE_BE16(RTE_ETHER_TYPE_IPV4)) {
        ...
        case IPPROTO_UDP:
            ...
            udp_hdr->dgram_cksum = upkt_ipv4_updtcp_checksum(ipv4_hdr, udp_hdr, sizeof(struct rte_udp_hdr), pkt->data, pkt->data_len);
            break;
        case IPPROTO_TCP:
            struct rte_tcp_hdr *tcp_hdr = &pkt->proto.tcp;
            tcp_hdr->cksum = 0;
            tcp_hdr->cksum = upkt_ipv4_updtcp_checksum(ipv4_hdr, tcp_hdr, sizeof(struct rte_tcp_hdr), pkt->data, pkt->data_len);
            break;
        case IPPROTO_ICMP:
            struct icmphdr *icmp_hdr = &pkt->proto.icmp;
            icmp_hdr->checksum = 0;
            uint32_t cksum = __rte_raw_cksum(icmp_hdr, sizeof(struct icmphdr), 0);
            if (pkt->data_len > 0 && pkt->data != NULL) {
                cksum = __rte_raw_cksum(pkt->data, pkt->data_len, cksum);
            }
        ...
        case IPPROTO_UDP:
            struct rte_udp_hdr *udp_hdr = &pkt->proto.udp;
            udp_hdr->dgram_cksum = 0;
            udp_hdr->dgram_cksum = upkt_ipv6_updtcp_checksum(ipv6_hdr, udp_hdr, sizeof(struct rte_udp_hdr), pkt->data, pkt->data_len);
            // RTE_LOG(INFO, NAT64_TEST, "(fix6)UDP checksum: %X\n", rte_be_to_cpu_16(udp_hdr->dgram_cksum));
            break;
        case IPPROTO_TCP:
            struct rte_tcp_hdr *tcp_hdr = &pkt->proto.tcp;
            tcp_hdr->cksum = 0;
            tcp_hdr->cksum = upkt_ipv6_updtcp_checksum(ipv6_hdr, tcp_hdr, sizeof(struct rte_tcp_hdr), pkt->data, pkt->data_len);
            break;
        case IPPROTO_ICMPV6:
            ...
            sum = __rte_raw_cksum(icmp6_hdr, sizeof(struct icmp6_hdr), sum);
...
push_packet(struct upkt *pkt) {
	...
	if (pkt->eth.ether_type == RTE_BE16(RTE_ETHER_TYPE_IPV4)) {
		...
		RTE_LOG(ERR, NAT64_TEST, "Usupported ether type %04X\n", pkt->eth.ether_type);
		return -1;
	}
	switch (proto) {
		case IPPROTO_UDP:
	...
	if (parse_packet(packet)) {
		RTE_LOG(ERR, NAT64_TEST, "Failed to parse packet after creation\n");
		return -1;
	}
			...
			.proto.udp =
			    {
			        ...
			        .dst_port = rte_cpu_to_be_16(53), /* DNS port */
			        .dgram_len = rte_cpu_to_be_16(sizeof(struct rte_udp_hdr) + 10), /* UDP header + data */
			    },
			    .data_len = 10,
			    .data = "0123456789"
		};
			...
			.ip.ipv6 =
				{
					...
					.payload_len = RTE_BE16(sizeof(struct rte_udp_hdr) + 10),
	},
.proto.udp =
	{
					.src_port = rte_cpu_to_be_16(12345),
					.dst_port = rte_cpu_to_be_16(53), /* DNS port */
					.dgram_len = rte_cpu_to_be_16(sizeof(struct rte_udp_hdr) + 10),
	},
	.data_len = 10,
	.data = "0123456789"
		};
		...
		rte_memcpy(&pkt_expected.ip.ipv6.src_addr, &pkt_expected.ip.ipv6.dst_addr, 16);
...
static struct upkt *
create_icmp_packet(const struct icmp_type_info_t *info, bool is_v6, uint8_t* prefix) {
    ...
    // Common fields
    rte_memcpy(&pkt->eth.dst_addr.addr_bytes, "\xff\xff\xff\xff\xff\xff", 6);
    rte_memcpy(&pkt->eth.src_addr.addr_bytes, "\x02\x00\x00\x00\x00\x00", 6);
    ...
    if (is_v6) {
        ...
        if (info->type6 == ICMP6_ECHO_REQUEST || info->type6 == ICMP6_ECHO_REPLY) {
        ...
        if (!info->embed_proto && (info->type6 == ICMP6_ECHO_REQUEST || info->type6 == ICMP6_ECHO_REPLY)) {
            ...
            pkt->data_len = 8;
            pkt->ip.ipv6.payload_len = rte_cpu_to_be_16(payload_len + 8);
        }
        // Set source/destination IPs
        if (info->from_ipv4) {
            rte_memcpy(&pkt->ip.ipv6.dst_addr, &config_data.mapping[0].ip6, 16);
            SET_IPV4_MAPPED_IPV6(&pkt->ip.ipv6.src_addr, prefix, &outer_ip4);
        } else {
            rte_memcpy(&pkt->ip.ipv6.src_addr, &config_data.mapping[0].ip6, 16);
            SET_IPV4_MAPPED_IPV6(&pkt->ip.ipv6.dst_addr, prefix, &outer_ip4);
        }
    } else {
        ...
        uint16_t total_len = sizeof(struct rte_ipv4_hdr) + sizeof(struct icmphdr);

        // For non-error messages that need data
        if (!info->embed_proto && (info->type == ICMP_ECHO || info->type == ICMP_ECHOREPLY)) {
            pkt->data = rte_malloc(NULL, 8, 0);
            if (!pkt->data) {
                rte_free(embedded_pkt);
                rte_free(pkt);
...
append_test_cases_from_mappings_icmp_more(struct test_case **test_case) {
    ...
    const struct icmp_type_info_t icmp_types[] = {
        { "Echo Request v4->v6", ICMP_ECHO, 0, ICMP6_ECHO_REQUEST, 0, true, false, 0, 0, 0, 0, false },
        { "Echo Reply v4->v6", ICMP_ECHOREPLY, 0, ICMP6_ECHO_REPLY, 0, true, false, 0, 0, 0, 0, false },
        { "Echo Request v6->v4", ICMP_ECHO, 0, ICMP6_ECHO_REQUEST, 0, false, false, 0, 0, 0, 0, false },
        { "Echo Reply v6->v4", ICMP_ECHOREPLY, 0, ICMP6_ECHO_REPLY, 0, false, false, 0, 0, 0, 0, false },
        { "Extended Echo Request v6->v4 (drop)", ICMP_ECHO, 0, ICMPV6_EXT_ECHO_REQUEST, 0, false, false, 0, 0, 0, 0, true },
        { "Extended Echo Reply v6->v4 (drop)", ICMP_ECHOREPLY, 0, ICMPV6_EXT_ECHO_REPLY, 0, false, false, 0, 0, 0, 0, true },
        
        // Destination Unreachable variations
        { "No Route v6->v4", ICMP_DEST_UNREACH, ICMP_HOST_UNREACH, ICMP6_DST_UNREACH, ICMP6_DST_UNREACH_NOROUTE, false, IPPROTO_UDP, 0, 0, 0, 0, false },
        { "Host Unreachable v6->v4", ICMP_DEST_UNREACH, ICMP_HOST_UNREACH, ICMP6_DST_UNREACH, ICMP6_DST_UNREACH_ADDR, false, IPPROTO_UDP, 0, 0, 0, 0, false },
        { "Protocol Unreachable v4->v6", ICMP_DEST_UNREACH, ICMP_PROT_UNREACH, ICMP6_PARAM_PROB, ICMP6_PARAMPROB_NEXTHEADER, true, IPPROTO_UDP, 0, 0, 0, 6, false },
        { "Port Unreachable v6->v4", ICMP_DEST_UNREACH, ICMP_PORT_UNREACH, ICMP6_DST_UNREACH, ICMP6_DST_UNREACH_NOPORT, false, IPPROTO_UDP, 0, 0, 0, 0, false },
        { "Admin Prohibited v6->v4", ICMP_DEST_UNREACH, ICMP_HOST_ANO, ICMP6_DST_UNREACH, ICMP6_DST_UNREACH_ADMIN, false, IPPROTO_UDP, 0, 0, 0, 0, false },
        { "Beyond Scope v6->v4", ICMP_DEST_UNREACH, ICMP_HOST_UNREACH, ICMP6_DST_UNREACH, ICMP6_DST_UNREACH_BEYONDSCOPE, false, IPPROTO_TCP, 0, 0, 0, 0, false },
        { "Source Route Failed v4->v6", ICMP_DEST_UNREACH, ICMP_SR_FAILED, ICMP6_DST_UNREACH, ICMP6_DST_UNREACH_NOROUTE, true, IPPROTO_UDP, 0, 0, 0, 0, false },
        { "Network Unknown v4->v6", ICMP_DEST_UNREACH, ICMP_NET_UNKNOWN, ICMP6_DST_UNREACH, ICMP6_DST_UNREACH_NOROUTE, true, IPPROTO_UDP, 0, 0, 0, 0, false },
        { "Host Unknown v4->v6", ICMP_DEST_UNREACH, ICMP_HOST_UNKNOWN, ICMP6_DST_UNREACH, ICMP6_DST_UNREACH_NOROUTE, true, IPPROTO_ICMP, 0, 0, 0, 0, false },
        { "Network Prohibited v4->v6", ICMP_DEST_UNREACH, ICMP_NET_ANO, ICMP6_DST_UNREACH, ICMP6_DST_UNREACH_ADMIN, true, IPPROTO_UDP, 0, 0, 0, 0, false },
        { "Host Prohibited v4->v6", ICMP_DEST_UNREACH, ICMP_HOST_ANO, ICMP6_DST_UNREACH, ICMP6_DST_UNREACH_ADMIN, true, IPPROTO_UDP, 0, 0, 0, 0, false },
        { "TOS & Network v4->v6", ICMP_DEST_UNREACH, ICMP_NET_UNR_TOS, ICMP6_DST_UNREACH, ICMP6_DST_UNREACH_NOROUTE, true, IPPROTO_UDP, 0, 0, 0, 0, false },
        { "TOS & Host v4->v6", ICMP_DEST_UNREACH, ICMP_HOST_UNR_TOS, ICMP6_DST_UNREACH, ICMP6_DST_UNREACH_NOROUTE, true, IPPROTO_UDP, 0, 0, 0, 0, false },
        { "Filtered v4->v6", ICMP_DEST_UNREACH, ICMP_PKT_FILTERED, ICMP6_DST_UNREACH, ICMP6_DST_UNREACH_ADMIN, true, IPPROTO_UDP, 0, 0, 0, 0, false },
        { "Precedence Violation v4->v6 (drop)", ICMP_DEST_UNREACH, ICMP_PREC_VIOLATION, ICMP6_DST_UNREACH, ICMP6_DST_UNREACH_ADMIN, true, IPPROTO_UDP, 0, 0, 0, 0, true},
        { "Precedence Cutoff v4->v6", ICMP_DEST_UNREACH, ICMP_PREC_CUTOFF, ICMP6_DST_UNREACH, ICMP6_DST_UNREACH_ADMIN, true, IPPROTO_UDP, 0, 0, 0, 0, false },
        
        // Packet Too Big with different MTU values
        { "PTB MTU=1280 v6->v4", ICMP_DEST_UNREACH, ICMP_FRAG_NEEDED, ICMP6_PACKET_TOO_BIG, 0, false, IPPROTO_UDP, 1260, 1280, 0, 0, false },
        { "PTB MTU=1500 v6->v4", ICMP_DEST_UNREACH, ICMP_FRAG_NEEDED, ICMP6_PACKET_TOO_BIG, 0, false, IPPROTO_UDP, 1260, 1500, 0, 0, false },
        { "PTB MTU=576 v6->v4", ICMP_DEST_UNREACH, ICMP_FRAG_NEEDED, ICMP6_PACKET_TOO_BIG, 0, false, IPPROTO_UDP, 547, 567, 0, 0, false },
        { "PTB MTU=0 v6->v4", ICMP_DEST_UNREACH, ICMP_FRAG_NEEDED, ICMP6_PACKET_TOO_BIG, 0, false, IPPROTO_UDP, 1260, 0, 0, 0, false },
        { "PTB MTU=65535 v6->v4", ICMP_DEST_UNREACH, ICMP_FRAG_NEEDED, ICMP6_PACKET_TOO_BIG, 0, false, IPPROTO_UDP, 1260, 65535, 0, 0, false },
       
        // Time Exceeded
        { "TTL Exceeded v6->v4", ICMP_TIME_EXCEEDED, ICMP_EXC_TTL, ICMP6_TIME_EXCEEDED, ICMP6_TIME_EXCEED_TRANSIT, false, IPPROTO_UDP, 0, 0, 0, 0, false },
        { "Fragment Reassembly v6->v4", ICMP_TIME_EXCEEDED, ICMP_EXC_FRAGTIME, ICMP6_TIME_EXCEEDED, ICMP6_TIME_EXCEED_REASSEMBLY, false, IPPROTO_UDP, 0, 0, 0, 0, false },
        
        // Parameter Problem with pointer translations - IPv4 to IPv6
        { "Header Error v4->v6 Version", ICMP_PARAMPROB, 0, ICMP6_PARAM_PROB, ICMP6_PARAMPROB_HEADER, true, IPPROTO_UDP, 0, 0, 0, 0, false },
        { "Header Error v4->v6 Traffic Class", ICMP_PARAMPROB, 0, ICMP6_PARAM_PROB, ICMP6_PARAMPROB_HEADER, true, IPPROTO_UDP, 0, 0, 1, 1, false },
        { "Header Error v4->v6 Flow Label", ICMP_PARAMPROB, 0, ICMP6_PARAM_PROB, ICMP6_PARAMPROB_HEADER, true, IPPROTO_TCP, 0, 0, 5, 2, true },
        { "Header Error v4->v6 Payload Length", ICMP_PARAMPROB, 0, ICMP6_PARAM_PROB, ICMP6_PARAMPROB_HEADER, true, IPPROTO_UDP, 0, 0, 2, 4, false },
        { "Header Error v4->v6 Next Header", ICMP_PARAMPROB, 0, ICMP6_PARAM_PROB, ICMP6_PARAMPROB_HEADER, true, IPPROTO_UDP, 0, 0, 9, 6, false },
        { "Header Error v4->v6 Hop Limit", ICMP_PARAMPROB, 0, ICMP6_PARAM_PROB, ICMP6_PARAMPROB_HEADER, true, IPPROTO_TCP, 0, 0, 8, 7, false },
        { "Header Error v4->v6 Source Address", ICMP_PARAMPROB, 0, ICMP6_PARAM_PROB, ICMP6_PARAMPROB_HEADER, true, IPPROTO_ICMP, 0, 0, 12, 8, false },
        { "Header Error v4->v6 Destination Address", ICMP_PARAMPROB, 0, ICMP6_PARAM_PROB, ICMP6_PARAMPROB_HEADER, true, IPPROTO_UDP, 0, 0, 16, 24, false },

        // Parameter Problem with pointer translations - IPv6 to IPv4
        { "Header Error v6->v4 ptr=0", ICMP_PARAMPROB, 0, ICMP6_PARAM_PROB, ICMP6_PARAMPROB_HEADER, false, IPPROTO_UDP, 0, 0, 0, 0, false },
        { "Header Error v6->v4 ptr=4", ICMP_PARAMPROB, 0, ICMP6_PARAM_PROB, ICMP6_PARAMPROB_HEADER, false, IPPROTO_UDP, 0, 0, 2, 4, false },
        { "Header Error v6->v4 ptr=6", ICMP_PARAMPROB, 0, ICMP6_PARAM_PROB, ICMP6_PARAMPROB_HEADER, false, IPPROTO_TCP, 0, 0, 9, 6, false },
        { "Header Error v6->v4 ptr=7", ICMP_PARAMPROB, 0, ICMP6_PARAM_PROB, ICMP6_PARAMPROB_HEADER, false, IPPROTO_UDP, 0, 0, 8, 7, false },
        { "Header Error v6->v4 ptr=8", ICMP_PARAMPROB, 0, ICMP6_PARAM_PROB, ICMP6_PARAMPROB_HEADER, false, IPPROTO_ICMP, 0, 0, 12, 8, false },
        { "Header Error v6->v4 ptr=20", ICMP_PARAMPROB, 0, ICMP6_PARAM_PROB, ICMP6_PARAMPROB_HEADER, false, IPPROTO_UDP, 0, 0, 12, 20, false },
        { "Next Header v6->v4", ICMP_DEST_UNREACH, ICMP_PROT_UNREACH, ICMP6_PARAM_PROB, ICMP6_PARAMPROB_NEXTHEADER, false, IPPROTO_UDP, 0, 0, 0, 0, false },
        
        // Drop cases - Non-translatable ICMPv6 messages
        { "MLD Query (drop)", 0, 0, MLD_LISTENER_QUERY, 0, false, IPPROTO_UDP, 0, 0, 0, 0, true },
        { "MLD Report (drop)", 0, 0, MLD_LISTENER_REPORT, 0, false, IPPROTO_UDP, 0, 0, 0, 0, true },
        { "MLD Done (drop)", 0, 0, MLD_LISTENER_REDUCTION, 0, false, IPPROTO_UDP, 0, 0, 0, 0, true },
        { "Router Solicitation (drop)", 0, 0, ND_ROUTER_SOLICIT, 0, false, IPPROTO_UDP, 0, 0, 0, 0, true },
        { "Router Advertisement (drop)", 0, 0, ND_ROUTER_ADVERT, 0, false, IPPROTO_UDP, 0, 0, 0, 0, true },
        { "Neighbor Solicitation (drop)", 0, 0, ND_NEIGHBOR_SOLICIT, 0, false, IPPROTO_UDP, 0, 0, 0, 0, true },
        { "Neighbor Advertisement (drop)", 0, 0, ND_NEIGHBOR_ADVERT, 0, false, IPPROTO_UDP, 0, 0, 0, 0, true },
        { "Redirect (drop)", 0, 0, ND_REDIRECT, 0, false, IPPROTO_UDP, 0, 0, 0, 0, true },
        { "Router Renumbering (drop)", 0, 0, ICMP6_ROUTER_RENUMBERING, 0, false, IPPROTO_UDP, 0, 0, 0, 0, true },
        
        // Invalid cases that should be dropped
        { "Invalid Parameter Problem ptr=40 (drop)", 0, 0, ICMP6_PARAM_PROB, ICMP6_PARAMPROB_HEADER, false, IPPROTO_UDP, 0, 0, 0, 40, true },
        { "Invalid Parameter Problem code=2 (drop)", 0, 0, ICMP6_PARAM_PROB, ICMP6_PARAMPROB_OPTION, false, IPPROTO_UDP, 0, 0, 0, 0, true },
        
        // Additional edge cases - ICMPv4 messages that should be dropped
        { "Source Quench v4->v6 (drop)", ICMP_SOURCE_QUENCH, 0, 0, 0, true, IPPROTO_UDP, 0, 0, 0, 0, true },
        // Obsolete ICMPv4 messages that should be dropped during v4->v6 translation
        { "Timestamp Request v4->v6 (drop)", ICMP_TIMESTAMP, 0, 0, 0, true, IPPROTO_UDP, 0, 0, 0, 0, true },
        { "Timestamp Reply v4->v6 (drop)", ICMP_TIMESTAMPREPLY, 0, 0, 0, true, IPPROTO_UDP, 0, 0, 0, 0, true },
        { "Information Request v4->v6 (drop)", ICMP_INFO_REQUEST, 0, 0, 0, true, IPPROTO_UDP, 0, 0, 0, 0, true },
        { "Information Reply v4->v6 (drop)", ICMP_INFO_REPLY, 0, 0, 0, true, IPPROTO_UDP, 0, 0, 0, 0, true },
        { "Address Mask Request v4->v6 (drop)", ICMP_ADDRESS, 0, 0, 0, true, IPPROTO_UDP, 0, 0, 0, 0, true },
        { "Address Mask Reply v4->v6 (drop)", ICMP_ADDRESSREPLY, 0, 0, 0, true, IPPROTO_UDP, 0, 0, 0, 0, true },
        // Test v6->v4 direction with invalid ICMPv6 type (should be dropped)
        { "Invalid ICMPv6 type v6->v4 (drop)", 0, 0, 255, 0, false, IPPROTO_UDP, 0, 0, 0, 0, true }
    };

    for (size_t i = 0; i < sizeof(icmp_types) / sizeof(struct icmp_type_info_t); ++i) {
        const struct icmp_type_info_t *info = &icmp_types[i];
        
        // For drop cases, create only one direction (v6->v4)
        if (info->should_drop) {
            struct upkt *pkt = create_icmp_packet(info, !info->from_ipv4, nat64_config->ipv6_prefixes[0].prefix);
            if (!pkt) {
                return -1;
            }
        ...
        struct upkt *pkt_v4 = create_icmp_packet(info, false, nat64_config->ipv6_prefixes[0].prefix);
        struct upkt *pkt_v6 = create_icmp_packet(info, true, nat64_config->ipv6_prefixes[0].prefix);

        if (!pkt_v4 || !pkt_v6) {
            if (pkt_v4) rte_free(pkt_v4);
            if (pkt_v6) rte_free(pkt_v6);
            return -1;
        }
        ...
        if (info->from_ipv4) {
            snprintf(buf_name, sizeof(buf_name),
                     "%s " IPv4_BYTES_FMT " -> v6", info->name, IPv4_BYTES_LE(pkt_v4->ip.ipv4.src_addr));
        append_test_case(test_case, *pkt_v4, *pkt_v6, buf_name);
        } else {
            snprintf(buf_name, sizeof(buf_name),
            "%s " IPv6_BYTES_FMT " -> v4", info->name, IPv6_BYTES(pkt_v6->ip.ipv6.src_addr)); 
...
append_test_cases_from_mappings_icmp(struct test_case **test_case) {
    ...
    for (uint32_t i = 0; i < config_data.count; i++) {
            ...
            .ip.ipv6 =
                {
                    ...
                    .src_addr = {0},
                    .vtc_flow = RTE_BE32(0x60000000),
                    .payload_len = RTE_BE16(sizeof(struct icmp6_hdr) + 10),
                },
            .proto.icmp6 =
                {
        ...
        // Change ICMP type for the second test case since it will be a response
...
struct nat64_module_config *nat64_config =
    ...
    struct upkt pkt = {
        .eth = {
            ...
            .src_addr.addr_bytes = "\x02\x00\x00\x00\x00\x00",
            .ether_type = RTE_BE16(RTE_ETHER_TYPE_IPV4),
        },
        .ip.ipv4 = {
            .version_ihl = RTE_IPV4_VHL_DEF,
            .total_length = RTE_BE16(sizeof(struct rte_ipv4_hdr) + sizeof(struct rte_udp_hdr) + 10),
            .time_to_live = DEFAULT_TTL,
            .next_proto_id = IPPROTO_UDP,
            .src_addr = outer_ip4,
        ...
        .proto.udp = {
            ...
            .dgram_len = RTE_BE16(18), // UDP header (8) + data (10)
        },
        .data_len = 10,
        .data = "0123456789"
    };
    ...
    struct upkt pkt_expected = {
        ...
        .ip.ipv6 = {
            .vtc_flow = RTE_BE32(0x60000000),
            .payload_len = RTE_BE16(sizeof(struct rte_udp_hdr) + 10), // UDP header (8) + data (10)
            .proto = IPPROTO_UDP,
            .hop_limits = DEFAULT_TTL,
        },
        .proto.udp = {
    ...
    // Set IPv6 addresses
    rte_memcpy(&pkt_expected.ip.ipv6.dst_addr, config_data.mapping[0].ip6, 16);
    SET_IPV4_MAPPED_IPV6(&pkt_expected.ip.ipv6.src_addr, nat64_config->ipv6_prefixes[0].prefix, &outer_ip4);
    ...
    TEST_ASSERT_EQUAL(count, 1, "Expected 1 packet output, got %d\n", count);

    struct packet *packet = packet_list_pop(&test_params.packet_front.output);
    ...
    // Compare packets
    int res = print_diff_upkt_and_rte_mbuf(&pkt_expected, packet_to_mbuf(packet));
    if (res != 0) {
        RTE_LOG(ERR, NAT64_TEST, "Packets differ. See above for details.\n");
...
static int process_test_case(struct test_case *tc) {
    packet_list_cleanup(&test_params.packet_front.input);
    packet_list_cleanup(&test_params.packet_front.output);
    packet_list_cleanup(&test_params.packet_front.drop);
    ...
    TEST_ASSERT_EQUAL(push_packet(&tc->pkt), 0, "%s: Failed to push packet \n", tc->name);
    ...
    while ((pkt_out = packet_list_pop(&test_params.packet_front.output)) != NULL) {
        // Parse again
...
test_nat64_generic(int (*tc_provider)(struct test_case **) ) {
    ...
    TEST_ASSERT_EQUAL(tc_provider(&test_cases), 0, "Failed to get test cases\n");
...
static int drop_test_case(struct test_case **test_cases) {
	append_test_case(test_cases, 
		(struct upkt){
			.eth =
				{
					.dst_addr.addr_bytes =
			...
			.ip.ipv4 =
				{
					...
					.dst_addr = RTE_BE32(RTE_IPV4(198, 58, 100, 1)),
				},
			.proto.udp =
				{
					.dst_port = RTE_BE16(9),
					.dgram_len = RTE_BE16(sizeof(struct rte_udp_hdr)),
				},
		},
    ...
    for (uint32_t i = 0; i < config_data.count; i++) {
        struct upkt pkt = {
            ...
            .proto.tcp = {
                ...
                .recv_ack = 0,
                .data_off = 0x50,  // 5 words * 4 bytes = 20 bytes TCP header
        ...
        struct upkt pkt_expected = {
            ...
            .ip.ipv6 = {
                .vtc_flow = RTE_BE32(0x60000000),
                .payload_len = RTE_BE16(sizeof(struct rte_tcp_hdr) + 10),
                .proto = IPPROTO_TCP,
                .hop_limits = DEFAULT_TTL,
            },
            .proto.tcp = {
        ...
        rte_memcpy(&pkt_expected.ip.ipv6.dst_addr, &config_data.mapping[i].ip6, 16);
        SET_IPV4_MAPPED_IPV6(&pkt_expected.ip.ipv6.src_addr, nat64_config->ipv6_prefixes[0].prefix, &outer_ip4);

        char buf[1024];
        snprintf(buf, 1023, "v4 -> v6 " IPv4_BYTES_FMT " -> " IPv6_BYTES_FMT,
                IPv4_BYTES_LE(outer_ip4),
                IPv6_BYTES(pkt_expected.ip.ipv6.dst_addr));
        append_test_case(test_case, pkt, pkt_expected, buf);

        snprintf(buf, 1023, "v6 -> v4 " IPv6_BYTES_FMT " -> " IPv4_BYTES_FMT,
                IPv6_BYTES(pkt_expected.ip.ipv6.src_addr),
                IPv4_BYTES_LE(outer_ip4));
