...
nat64_handle_configure(
	...
	struct nat64_module_config *config = (struct nat64_module_config *)
		rte_malloc(NULL, sizeof(struct nat64_module_config), 0);
	...
	size_t prfxs_size = sizeof(prfxs);
	config->ipv6_prefixes = rte_malloc(NULL, prfxs_size, 0);
	if (config->ipv6_prefixes == NULL) {
		RTE_LOG(ERR,
			NAT64,
			"Failed to allocate memory for ipv6 prefixes\n");
		return -ENOMEM;
	}

	rte_memcpy(config->ipv6_prefixes, prfxs, prfxs_size);
...
/**
 * Validates IPv4/IPv6 fragment parameters according to RFC7915
 *
 * @param frag_offset Fragment offset in bytes
 * @param frag_size Fragment size in bytes
 * @param total_len Total packet length
 * @param more_fragments More fragments flag
 * @param is_icmp Whether the packet is ICMP
...
validate_fragment_params(
    ...
    if (frag_offset % 8 != 0) {
        LOG_DBG(NAT64, "Invalid fragment offset (not multiple of 8): %u\n", frag_offset);
        return -1;
    }

    // Non-last fragments must be multiple of 8 bytes
    if (more_fragments && (frag_size % 8 != 0)) {
        LOG_DBG(NAT64, "Non-last fragment size not multiple of 8: %u\n", frag_size);
        return -1;
    }
 ...
 *
 * This function implements the stateless translation of ICMPv6 messages to ICMPv4
 * as specified in RFC7915. It handles various ICMPv6 message types including:
 ...
 * headers according to section 5.3. This includes adjusting MTU values, translating
 * IP headers, and recalculating checksums.
 *
...
icmp_v6_to_v4(
    ...
    case ICMP6_PACKET_TOO_BIG:
    	...
    	uint32_t mtu = rte_be_to_cpu_32(icmpHeader->icmp6_mtu);
    	LOG_DBG(NAT64, "Original ICMPv6 MTU: %u\n", mtu);
    	
    	// RFC7915 Section 5.2: Handle MTU adjustment for Packet Too Big messages
    	...
    	if (mtu == 0) {
    		...
    		mtu = nat64_config->mtu;
    	}
    
    	// Calculate header size difference
    	uint16_t delta = sizeof(struct rte_ipv6_hdr) - sizeof(struct rte_ipv4_hdr);
    	
    	// RFC7915: Adjust MTU for header size difference
    	uint16_t adjusted_mtu = mtu - delta;
    	
    	if (nat64_config->mtu6 > 0) {
    		adjusted_mtu = RTE_MIN(adjusted_mtu, nat64_config->mtu6 - delta);
    	}
    	
    	if (nat64_config->mtu > 0) {
    		// Account for IPv4->IPv6 translation overhead
    		adjusted_mtu = RTE_MIN(adjusted_mtu, nat64_config->mtu);
    	}
    	...
    	LOG_DBG(NAT64,
    		...
    		"  - Config IPv4 MTU: %u\n"
    		"  - Adjusted MTU: %u\n",
    		mtu, nat64_config->mtu6,
    		nat64_config->mtu, adjusted_mtu);
    	...
    	struct icmp *icmp_hdr = (struct icmp *)icmpHeader;
    ...
    case ICMP6_PARAM_PROB:
    	...
    	case ICMP6_PARAMPROB_HEADER:
    		type = ICMP_PARAMPROB;
    		code = 0;  // RFC7915: Set Code to 0 (Erroneous header field encountered)
    		
    		// RFC7915 Figure 6: Pointer translation from IPv6 to IPv4
    		uint32_t ptr = rte_be_to_cpu_32(icmpHeader->icmp6_pptr);
    		
    		LOG_DBG(NAT64, "Translating ICMPv6 Parameter Problem pointer: %u\n", ptr);
    		
    		switch (ptr) {
    			case 0:  // Version/Traffic Class
    				LOG_DBG(NAT64, "IPv6 Version/Traffic Class -> IPv4 Version/IHL\n");
    				ptr = 0;
    				break;
    			case 1:  // Traffic Class/Flow Label
    				LOG_DBG(NAT64, "IPv6 Traffic Class/Flow Label -> IPv4 Type Of Service\n");
    				ptr = 1;
    				break;
    			case 4:  // Payload Length
    			case 5:
    				LOG_DBG(NAT64, "IPv6 Payload Length -> IPv4 Total Length\n");
    				ptr = 2;
    				break;
    			case 6:  // Next Header
    				LOG_DBG(NAT64, "IPv6 Next Header -> IPv4 Protocol\n");
    				ptr = 9;
    				break;
    			case 7:  // Hop Limit
    				LOG_DBG(NAT64, "IPv6 Hop Limit -> IPv4 Time to Live\n");
    				ptr = 8;
    				break;
    			case 8:  // Source Address (first byte)
    			case 9: case 10: case 11: case 12: case 13: case 14: case 15:
    			case 16: case 17: case 18: case 19: case 20: case 21: case 22: case 23:
    				LOG_DBG(NAT64, "IPv6 Source Address -> IPv4 Source Address\n");
    				ptr = 12;
    				break;
    			case 24:  // Destination Address (first byte)
    			case 25: case 26: case 27: case 28: case 29: case 30: case 31:
    			case 32: case 33: case 34: case 35: case 36: case 37: case 38: case 39:
    				LOG_DBG(NAT64, "IPv6 Destination Address -> IPv4 Destination Address\n");
    				ptr = 16;
    				break;
    			case 2:  // Flow Label
    			case 3:
    				LOG_DBG(NAT64, "IPv6 Flow Label has no IPv4 equivalent, dropping packet\n");
    				return -1;
    			case 40:  // Extension Headers
    			default:
    				LOG_DBG(NAT64, "Parameter Problem pointer not translatable: %u\n", ptr);
    				return -1;
    		}
    		
    		LOG_DBG(NAT64, "Translated Parameter Problem pointer to IPv4 offset: %u\n", ptr);
    		icmpHeader->icmp6_pptr = rte_cpu_to_be_32(ptr << 24); // set first byte to ptr and zeroed other bytes (reserved 6-8). TODO: support RFC4884.
    		break;
		...
		case ICMP6_PARAMPROB_NEXTHEADER:
			...
			LOG_DBG(NAT64, "Next Header Problem -> Protocol Unreachable\n");
			break;
    		
    	case ICMP6_PARAMPROB_OPTION:
    		// RFC7915: Silently drop packets with unrecognized IPv6 options
    		LOG_DBG(NAT64, "Dropping packet with unrecognized IPv6 option\n");
    		return -1;
    		
    	default:
    		LOG_DBG(NAT64, "Unknown Parameter Problem code: %u\n", code);
    		return -1;
    	}
    	break;
    ...
    case ND_REDIRECT:
        ...
        LOG_DBG(NAT64, "Single-hop ICMPv6 message (type %d), dropping\n", type);
        return -1;
    
    // RFC7915 Section 4.2: Information Request/Reply (Type 15 and Type 16): Obsoleted in ICMPv6. Silently drop.
    case ICMP6_ROUTER_RENUMBERING:
        LOG_DBG(NAT64, "Router Renumbering message (type %d), dropping\n", type);
        return -1;
    
    // RFC7915 Section 4.2: Timestamp and Timestamp Reply (Type 13 and Type 14): Obsoleted in ICMPv6. Silently drop.
    // RFC7915 Section 4.2: Address Mask Request/Reply (Type 17 and Type 18): Obsoleted in ICMPv6. Silently drop.
    // RFC7915 Section 4.2: Information Request/Reply (Type 15 and Type 16): Obsoleted in ICMPv6. Silently drop.
    
    default:
        LOG_DBG(NAT64, "Unknown ICMPv6 message type: %d, dropping\n", type);
        return -1;
    }
    ...
    bool is_error = (type == ICMP_DEST_UNREACH ||
    		type == ICMP_TIME_EXCEEDED ||
    		type == ICMP_PARAMPROB);
   
    if (is_error) {
    	LOG_DBG(NAT64, "Translating ICMP error message with embedded packet\n");
   
    	// Get offset to embedded packet
    	uint16_t embedded_offset = packet->transport_header.offset + sizeof(struct icmp6_hdr);
   
    	// RFC7915: Validate minimum length requirements
    	uint16_t remaining_len = rte_pktmbuf_data_len(mbuf) - embedded_offset;
    	if (remaining_len < sizeof(struct rte_ipv6_hdr)) {
    		LOG_DBG(NAT64, "ICMP error message too short (len=%u, min=%zu)\n",
    			remaining_len, sizeof(struct rte_ipv6_hdr));
    		return -1;
    	}
   
    	// Extract the embedded IPv6 header
    	struct rte_ipv6_hdr *ipv6_payload_header = rte_pktmbuf_mtod_offset(
    		mbuf,
    		struct rte_ipv6_hdr *,
    		embedded_offset
    	);
    	...
    	if (!ipv6_payload_header) {
    		RTE_LOG(ERR, NAT64, "Failed to get embedded IPv6 header\n");
    		return -1;
    	}
   
    	// RFC7915: Validate embedded packet length
    	uint16_t embedded_total_len = rte_be_to_cpu_16(new_ipv4_header->total_length) - rte_ipv4_hdr_len(new_ipv4_header) - sizeof(struct icmp6_hdr);
   
    	if (remaining_len < embedded_total_len) {
    		LOG_DBG(NAT64,
    			"Embedded packet length (%u) exceeds remaining space (%u)\n",
    			embedded_total_len, remaining_len);
    		return -1;
    	}
   
    	// RFC7915: Check for nested ICMP errors (not allowed)
    	if (ipv6_payload_header->proto == IPPROTO_ICMPV6) {
    		struct icmp6_hdr *embedded_icmp = rte_pktmbuf_mtod_offset(
    			mbuf,
    			struct icmp6_hdr *,
    			embedded_offset + sizeof(struct rte_ipv6_hdr)
    		);
   
    		if (!embedded_icmp) {
    			RTE_LOG(ERR, NAT64, "Failed to get embedded ICMPv6 header\n");
    			return -1;
    		}
   
    		if (embedded_icmp->icmp6_type < 128) {  // Error message
    			LOG_DBG(NAT64, "Nested ICMP error messages not allowed\n");
    			return -1;
    		}
    	}
	...
	// Skip extension headers
        while (next_header == IPPROTO_HOPOPTS ||
            ...
            struct ipv6_ext_2byte *ext_hdr = rte_pktmbuf_mtod_offset(
                mbuf,
                struct ipv6_ext_2byte *,
                packet->transport_header.offset + sizeof(struct icmp6_hdr) + offset
            );
            
            if (!ext_hdr) {
                RTE_LOG(ERR, NAT64, "Failed to get IPv6 extension header\n");
                return -1;
            }
        ...
        if (next_header == IPPROTO_FRAGMENT) {
            ...
            struct ipv6_ext_fragment *frag_hdr = rte_pktmbuf_mtod_offset(
                mbuf,
            ...
            if (!frag_hdr) {
                RTE_LOG(ERR, NAT64, "Failed to get IPv6 fragment header\n");
                return -1;
            }
		...
		if (delta < 0) {
            // IPv4 header is larger than IPv6 header, strange - drop
			LOG_DBG(NAT64, "ICMPv6 payload IPv4 header is larger than IPv6 headers\n");
			return -1;
        }

			// Create the IPv4 header in place of the IPv6 header
	struct rte_ipv4_hdr *new_ipv4_payload_header = rte_pktmbuf_mtod_offset(
		mbuf,
		struct rte_ipv4_hdr *,
		packet->transport_header.offset + sizeof(struct icmp6_hdr) + delta
	);
	...
	if (!new_ipv4_payload_header) {
        ...
        new_ipv4_payload_header->type_of_service =
            (rte_be_to_cpu_32(ipv6_payload_header->vtc_flow) >> 20) & 0xFF;
        
        // Set the total length
        uint16_t payload_length = rte_be_to_cpu_16(ipv6_payload_header->payload_len);
        new_ipv4_payload_header->total_length =
        ...
        if (is_fragmented) {
            struct ipv6_ext_fragment *frag_hdr = rte_pktmbuf_mtod_offset(
                mbuf,
                struct ipv6_ext_fragment *,
                packet->transport_header.offset + sizeof(struct icmp6_hdr) + sizeof(struct rte_ipv6_hdr)
            );
            
            new_ipv4_payload_header->packet_id = frag_hdr->identification;
            
            uint16_t frag_data = rte_be_to_cpu_16(frag_hdr->offset_flag);
            uint16_t frag_offset = (frag_data & RTE_IPV6_EHDR_FO_MASK) >> RTE_IPV6_EHDR_FO_SHIFT;
            uint16_t frag_flags = frag_data & RTE_IPV6_EHDR_MF_MASK;
            
            new_ipv4_payload_header->fragment_offset =
                rte_cpu_to_be_16((frag_offset << 3) | (frag_flags ? RTE_IPV4_HDR_MF_FLAG : 0));
        } else {
        ...
        new_ipv4_payload_header->time_to_live = ipv6_payload_header->hop_limits;
        ...
        // If the embedded packet contains a transport layer header, translate it too
        if (!is_fragmented || (is_fragmented && (new_ipv4_payload_header->fragment_offset & RTE_BE16(RTE_IPV4_HDR_OFFSET_MASK)) == 0)) {
            uint16_t transport_offset = packet->transport_header.offset +
                                       sizeof(struct icmp6_hdr) +
                                       sizeof(struct rte_ipv4_hdr) + delta;
            
            switch (new_ipv4_payload_header->next_proto_id) {
                case IPPROTO_ICMPV6:
                    // Embedded ICMPv6 header needs to be translated to ICMPv4
                    struct icmp6_hdr *embedded_icmp6 = rte_pktmbuf_mtod_offset(
                        mbuf, struct icmp6_hdr *, transport_offset
                    );
                    ...
                    // Comprehensive translation of embedded ICMPv6 to ICMPv4
                    switch (embedded_icmp6->icmp6_type) {
                        ...
                        case ICMP6_ECHO_REPLY:
						embedded_icmp6->icmp6_type = ICMP_ECHOREPLY;
                            break;
                            
                        default:
                            // Other ICMPv6 types not translatable
                            LOG_DBG(NAT64, "Embedded ICMPv6 type not translatable: %d\n", embedded_icmp6->icmp6_type);
							return -1;
                            break;
                    }
                    ...
                    new_ipv4_payload_header->next_proto_id = IPPROTO_ICMP;
                    
                    // Recalculate the ICMP checksum
                    struct icmp *embedded_icmp4 = (struct icmp *)embedded_icmp6;
                    embedded_icmp4->icmp_cksum = 0;
                    
                    // Calculate the ICMP checksum - use the correct length
                    // uint16_t icmp_len = payload_length;
                    // if (icmp_len > sizeof(struct icmp)) {
                    //     icmp_len -= sizeof(struct rte_ipv4_hdr);
                    // } else {
                    //     icmp_len = sizeof(struct icmp);
                    // }
                    
                    embedded_icmp4->icmp_cksum = ~rte_raw_cksum(embedded_icmp4, payload_length);
                ...
                case IPPROTO_UDP:
                    ...
                    if (!udp_hdr) {
                        RTE_LOG(ERR, NAT64, "Failed to get embedded UDP header\n");
                        return -1;
                    }
                    
                    udp_hdr->dgram_cksum = 0;
                    udp_hdr->dgram_cksum = rte_ipv4_udptcp_cksum(new_ipv4_payload_header, udp_hdr);
                    break;
                ...
                case IPPROTO_TCP:
                    ...
                    if (!tcp_hdr) {
                        RTE_LOG(ERR, NAT64, "Failed to get embedded TCP header\n");
                        return -1;
                    }
                    
                    tcp_hdr->cksum = 0;
                    tcp_hdr->cksum = rte_ipv4_udptcp_cksum(new_ipv4_payload_header, tcp_hdr);
                    break;
            }
        }
    new_ipv4_payload_header->hdr_checksum = rte_ipv4_cksum(new_ipv4_payload_header);
    ...
    char *dst = rte_pktmbuf_mtod_offset(
	    mbuf,
	    char *, packet->transport_header.offset + sizeof(struct icmp6_hdr));
	...
	new_ipv4_header->total_length = rte_cpu_to_be_16(rte_be_to_cpu_16(new_ipv4_header->total_length) - delta);
    ...
    uint16_t ipv4_total_len = rte_be_to_cpu_16(new_ipv4_header->total_length);
    ...
    LOG_DBG(NAT64,
        ...
        "  - ICMP length: %u\n",
        ipv4_total_len, ipv4_hdr_len, icmp_len);
    ...
    LOG_DBG(NAT64,
        ...
        "  - Message type: %u\n"
        "  - Message code: %u\n",
        rte_be_to_cpu_16(icmp_hdr->icmp_cksum), icmp_hdr->icmp_type, icmp_hdr->icmp_code);
 ...
 *
 * This function processes all IPv6 extension headers in a packet, including
 * fragment headers, and updates the packet structure accordingly.
 * It follows RFC7915 requirements for handling extension headers.
 *
 * @param nat64_config Pointer to the NAT64 module configuration
 * @param packet Pointer to the packet structure
 * @param next_header Pointer to store the next header type after all extensions
 * @param is_fragmented Pointer to store whether the packet is fragmented
 * @param frag_offset Pointer to store the fragment offset if fragmented
 * @param frag_flags Pointer to store the fragment flags if fragmented
 * @param frag_id Pointer to store the fragment ID if fragmented
 * @param ext_hdrs_len Pointer to store the total length of all extension headers
...
process_ipv6_extension_headers(
    ...
    *ext_hdrs_len = 0;
    ...
    uint16_t current_offset = packet->network_header.offset + sizeof(struct rte_ipv6_hdr);
    ...
    while (1) {
    	...
    	if (*next_header != IPPROTO_HOPOPTS &&
    	    ...
    	    *next_header != IPPROTO_ESP) {     // Encapsulating Security Payload
    		break;
    	}
   
    	// Get the extension header
    	void *ext_hdr = rte_pktmbuf_mtod_offset(mbuf, void *, current_offset);
    	if (!ext_hdr) {
    		RTE_LOG(ERR, NAT64, "Failed to get IPv6 extension header at offset %u\n", current_offset);
    		return -1;
    	}
    	...
    	LOG_DBG(NAT64,
    		...
    		*next_header, current_offset, *ext_hdrs_len);
    	...
    	switch (*next_header) {
    		...
    		case IPPROTO_DSTOPTS: {
    			...
    			struct ipv6_ext_2byte *hdr = (struct ipv6_ext_2byte *)ext_hdr;
    			uint8_t hdr_len = (hdr->size + 1) * 8;
   
    			// Validate header length
    			if (current_offset + hdr_len > rte_pktmbuf_data_len(mbuf)) {
    				LOG_DBG(NAT64, "Extension header exceeds packet bounds\n");
    				return -1;
    			}
    		...
    		case IPPROTO_ROUTING: {
    			...
    			if (((uint8_t *)hdr)[2] == 0) {
    				LOG_DBG(NAT64, "Dropping packet with Type 0 Routing Header\n");
    				return -1;
    			}
    			...
    			// Validate header length
    			if (current_offset + hdr_len > rte_pktmbuf_data_len(mbuf)) {
    				LOG_DBG(NAT64, "Routing header exceeds packet bounds\n");
    				return -1;
    			}
    			...
    			*ext_hdrs_len += hdr_len;
    			...
    			LOG_DBG(NAT64,
    				...
    				"  - Length: %u bytes\n"
    				"  - Next header: %u\n",
    				((uint8_t *)hdr)[2], hdr_len, hdr->next_type);
    			break;
    		}
   
    		case IPPROTO_FRAGMENT: {
    			// RFC7915: Process Fragment Header
    			struct ipv6_ext_fragment *frag_hdr = (struct ipv6_ext_fragment *)ext_hdr;
   
    			// Validate fragment header
    			if (current_offset + sizeof(*frag_hdr) > rte_pktmbuf_data_len(mbuf)) {
    				LOG_DBG(NAT64, "Fragment header exceeds packet bounds\n");
    				return -1;
    			}
    			...
    			*frag_id = frag_hdr->identification;
    			...
    			if (frag_hdr->next_type == IPPROTO_ICMPV6) {
    				LOG_DBG(NAT64, "Dropping fragmented ICMPv6 packet\n");
    				return -1;
    			}
    			...
    			*ext_hdrs_len += sizeof(*frag_hdr);
    			...
    			LOG_DBG(NAT64,
    				...
    				"  - ID: 0x%x\n"
    				"  - Next header: %u\n",
    				*frag_offset, *frag_flags, *frag_id, *next_header);
    			break;
    		}
    		...
    		case IPPROTO_ESP: {
    			...
    			LOG_DBG(NAT64, "Dropping packet with IPsec header (type %u)\n", *next_header);
    			return -1;
    		}
   
    		default:
    			RTE_LOG(ERR, NAT64, "Unexpected extension header type: %u\n", *next_header);
    			return -1;
    	}
    }
    ...
    LOG_DBG(NAT64, "Finished processing IPv6 extension headers: next_header=%u, is_fragmented=%u, ext_hdrs_len=%u\n",
           *next_header, *is_fragmented, *ext_hdrs_len);
...
nat64_handle_v6(
	...
	if (!ipv6Header) {
	   ...
	   uint8_t is_fragmented = 0;
	   uint8_t next_header = ipv6Header->proto;
	  ...
	  LOG_DBG(NAT64, "Processing IPv6 extension headers\n"); 
	   ...
	   if (is_fragmented) {
	       uint16_t total_len = rte_be_to_cpu_16(ipv6Header->payload_len);
	...
	// Update transport header offset to account for all extension headers
	packet->transport_header.offset = packet->network_header.offset +
		sizeof(struct rte_ipv6_hdr) + ext_hdrs_len;
	...
	if (is_fragmented) {
		...
		new_ipv4_header->fragment_offset = 
			rte_cpu_to_be_16((frag_offset << 3) | (frag_flags ? RTE_IPV4_HDR_MF_FLAG : 0));
	} else {
	...
	if (ipv6Header->proto == IPPROTO_FRAGMENT) {
		rte_memcpy(&new_ipv4_header->dst_addr, &ipv6Header->dst_addr[12], sizeof(uint32_t));
	}
	...
	if (ipv6Header->proto == IPPROTO_ICMPV6) {
		...
		if (result) {
			LOG_DBG(NAT64, "ICMP translation failed, dropping packet\n");
			return -1;
		}
	} else if (ipv6Header->proto == IPPROTO_UDP) {
		...
		if (!udp_hdr) {
			RTE_LOG(ERR, NAT64, "Failed to get UDP header from mbuf\n");
			return -1;
		}
		
		udp_hdr->dgram_cksum = 0;
		udp_hdr->dgram_cksum = rte_ipv4_udptcp_cksum_mbuf(mbuf, new_ipv4_header, packet->transport_header.offset);
		LOG_DBG(NAT64, "UDP checksum calculated: 0x%04X\n", rte_be_to_cpu_16(udp_hdr->dgram_cksum));

	} else if (ipv6Header->proto == IPPROTO_TCP) {
		...
		if (!tcp_hdr) {
			RTE_LOG(ERR, NAT64, "Failed to get TCP header from mbuf\n");
			return -1;
		}
		
		tcp_hdr->cksum = 0;
		tcp_hdr->cksum = rte_ipv4_udptcp_cksum_mbuf(mbuf, new_ipv4_header, packet->transport_header.offset);
	}
	// copy l2 header
	rte_memcpy(
...
copy_ipv4_to_ipv6_hdr(
	...
	if (is_fragmented) {
		...
		uint16_t frag_offset = rte_be_to_cpu_16(ipv4_header->fragment_offset);
		uint16_t more_fragments = !!(frag_offset & RTE_IPV4_HDR_MF_FLAG);
		uint16_t offset_value = (frag_offset & RTE_IPV4_HDR_OFFSET_MASK) << 3;

		LOG_DBG(NAT64,
			"IPv4 fragment: offset=%u, more_fragments=%u, id=0x%x\n",
			offset_value, more_fragments, rte_be_to_cpu_16(ipv4_header->packet_id));

		// RFC7915: Fragment offset must be a multiple of 8
		if (offset_value % 8) {
			LOG_DBG(NAT64, "Invalid IPv4 fragment offset (not multiple of 8): %u\n", offset_value);
			return -1;
		}
		...
		if (!frag_hdr) {
			LOG_DBG(NAT64, "Failed to get space for IPv6 fragment header\n");
			return -1;
		}

		// Set fragment header fields
		frag_hdr->next_header = ipv4_header->next_proto_id == IPPROTO_ICMP ?
			IPPROTO_ICMPV6 : ipv4_header->next_proto_id;
		frag_hdr->reserved = 0;
	
		// Set fragment offset and M flag using DPDK macros
		frag_hdr->frag_data = rte_cpu_to_be_16(
			RTE_IPV6_SET_FRAG_DATA(
		...
		frag_hdr->id = rte_cpu_to_be_32(rte_be_to_cpu_16(ipv4_header->packet_id));

		LOG_DBG(NAT64,
			"Created IPv6 fragment header: next_header=%u, offset=%u, M=%u, id=0x%x\n",
...
icmp_v4_to_v6(
	...
	struct icmp *icmpHeader = rte_pktmbuf_mtod_offset(
	...
	case ICMP_UNREACH:
		...
		case ICMP_PROT_UNREACH:
			...
			((struct icmp6_hdr *)icmpHeader)->icmp6_pptr = rte_be_to_cpu_32(6);
			break;
		...
		case ICMP_FRAG_NEEDED:
			...
			uint16_t mtu = rte_be_to_cpu_16(icmpHeader->icmp_nextmtu);
			
			// RFC7915: If MTU is 0, router doesn't implement RFC1191
			if (mtu == 0) {
				// TODO: RFC1191 values
				// Use configured MTU
				mtu = nat64_config->mtu;
			...
			if (nat64_config->mtu6 > 0) {
				mtu = RTE_MIN(mtu, nat64_config->mtu6);
			}
			if (nat64_config->mtu > 0) {
				mtu = RTE_MIN(mtu, nat64_config->mtu + 20);
			}
			// RFC7915: MTU must not be less than IPv6 minimum (1280)
			...
			LOG_DBG(NAT64,
				...
				"  - Adjusted MTU: %u\n"
				"  - Config IPv6 MTU: %u\n",
				rte_be_to_cpu_16(icmpHeader->icmp_nextmtu),
				mtu,
				nat64_config->mtu6);
	...
	case ICMP_PARAMPROB:
		...
		uint8_t ptr = icmpHeader->icmp_pptr;
		switch (ptr) {
		case 0:
		case 1:
			// clang-format off
		...
		((struct icmp6_hdr *)icmpHeader)->icmp6_pptr = rte_be_to_cpu_32(ptr);
		break;
	...
	if (type < 128) {
	struct rte_ipv4_hdr *ipv4_payload_header = rte_pktmbuf_mtod_offset(
		mbuf,
		struct rte_ipv4_hdr *,
	...
	if (delta < 0) {
		RTE_LOG(ERR,
			NAT64,
			"Failed to translate icmp payload with ipv4 header "
			"with options\n");
		return -1;
	}

	uint16_t payload_len = rte_be_to_cpu_16(new_ipv6_header->payload_len);
	...
	new_ipv6_header->payload_len = rte_cpu_to_be_16(new_payload_len);

	uint16_t buff_delta = delta + (mtu_overflow < 0 ? mtu_overflow : 0
	...
	if (buff_delta > 0) {
		if (!rte_pktmbuf_append(mbuf, buff_delta)) {
			RTE_LOG(ERR,
				NAT64,
				"Failed to append mbuf for icmpv6 payload\n");
			return -1;
		}
	} else {
		if (rte_pktmbuf_trim(mbuf, -buff_delta)) {
			RTE_LOG(ERR,
				NAT64,
				"Failed to trim mbuf for icmpv6 payload\n");
			return -1;
		}
	}
	...
	if (copy_ipv4_to_ipv6_hdr(
		...
		RTE_LOG(ERR,
			NAT64,
			"Failed to copy icmp payload ipv4 to ipv6 header\n");
		return -1;
	}
	if (!skip_translate) {
		...
		case IPPROTO_ICMP:
			...
			struct rte_icmp_hdr *icmpHeaderPayload =
				rte_pktmbuf_mtod_offset(
					mbuf,
					struct rte_icmp_hdr *,
					payload_offset
				);
			if (icmpHeaderPayload->icmp_type == ICMP_ECHO) {
				icmpHeaderPayload->icmp_type = ICMP6_ECHO_REQUEST;
			} else if (icmpHeaderPayload->icmp_type == ICMP_ECHOREPLY) {
				icmpHeaderPayload->icmp_type = ICMP6_ECHO_REPLY;
			} else {
				RTE_LOG(ERR,
					NAT64,
					"Unknown icmp type %d in icmp "
					"payload\n",
					icmpHeaderPayload->icmp_type);
				return -1;
			}

			// Recalculate ICMP checksum for IPv6 embeded
			icmpHeaderPayload->icmp_cksum = 0;
			uint32_t sum = rte_ipv6_phdr_cksum(new_ipv6_payload_header, 0);
			sum = __rte_raw_cksum(
				icmpHeaderPayload, rte_be_to_cpu_16(new_ipv6_payload_header->payload_len), sum
			);
		
			icmpHeaderPayload->icmp_cksum = ~__rte_raw_cksum_reduce(sum);
			break;

		case IPPROTO_UDP:
			struct rte_udp_hdr *udpHeader = rte_pktmbuf_mtod_offset(
				mbuf, struct rte_udp_hdr *, payload_offset
			);
			...
			udpHeader->dgram_cksum = rte_ipv6_udptcp_cksum_mbuf(
				mbuf, new_ipv6_payload_header, payload_offset
			);
			break;

		case IPPROTO_TCP:
			struct rte_tcp_hdr *tcpHeader = rte_pktmbuf_mtod_offset(
				mbuf, struct rte_tcp_hdr *, payload_offset
			);
			...
			tcpHeader->cksum = 0;
			tcpHeader->cksum = rte_ipv6_udptcp_cksum_mbuf(
				mbuf, new_ipv6_payload_header, payload_offset
			);
			break;
	...
	sum = __rte_raw_cksum(
		icmpHeader, rte_be_to_cpu_16(new_ipv6_header->payload_len), sum
	);

	icmpHeader->icmp_cksum = ~__rte_raw_cksum_reduce(sum);
...
nat64_handle_v4(
	...
	if (ihl > RTE_IPV4_MIN_IHL) {
		uint8_t *options = (uint8_t *)(ipv4Header + 1);
		uint8_t options_len = (ihl - RTE_IPV4_MIN_IHL) * RTE_IPV4_IHL_MULTIPLIER;
		uint8_t *options_end = options + options_len;

		LOG_DBG(NAT64, "Processing IPv4 options: IHL=%u, options_len=%u\n",
			ihl, options_len);
		...
		while (options < options_end) {
			...
			if ((option_type == IPOPT_LSRR) || (option_type == IPOPT_SSRR)) {
				LOG_DBG(NAT64, "Source route option found (type 0x%x), sending ICMP error\n",
					option_type);
				
				// RFC7915: Send ICMP error for source route options
				struct rte_icmp_hdr *icmp_hdr = (struct rte_icmp_hdr *)
					((char *)ipv4Header + sizeof(struct rte_ipv4_hdr));
				...
				LOG_DBG(NAT64, "Dropping packet with source route option\n");
				return -1;
			}
			...
			if (option_type == RTE_IPV4_HDR_OPT_NOP) {
				...
				if (options + 1 >= options_end) {
					LOG_DBG(NAT64, "Malformed IPv4 options: option extends beyond options end\n");
					return -1;
				}
				uint8_t option_len = options[1];
				if (option_len < 2 || options + option_len > options_end) {
					LOG_DBG(NAT64, "Invalid IPv4 option length: %u\n", option_len);
					return -1;
				}
				LOG_DBG(NAT64, "Skipping option type 0x%x, length %u\n",
	...
	int16_t delta =
	   ...
	   uint16_t frag_data = rte_be_to_cpu_16(ipv4Header->fragment_offset);
	   uint16_t frag_offset = (frag_data & RTE_IPV4_HDR_OFFSET_MASK) << 3;
	   bool more_fragments = !!(frag_data & RTE_IPV4_HDR_MF_FLAG);
	   uint8_t is_fragmented = frag_data & (RTE_IPV4_HDR_MF_FLAG | RTE_IPV4_HDR_OFFSET_MASK);

	   if (is_fragmented) {
	       // Calculate fragment size
	       uint16_t total_len = rte_be_to_cpu_16(ipv4Header->total_length);
	       uint16_t header_len = (ipv4Header->version_ihl & RTE_IPV4_HDR_IHL_MASK) * 4;
	       ...
	       if (validate_fragment_params(
	               ...
	               total_len,
	               more_fragments,
	               ipv4Header->next_proto_id == IPPROTO_ICMP) != 0) {
	           return -1;
	       }
	...
	if (delta >= 0) {
		...
		// RFC7915 1.2 ( -> .. -> RFC2765 1.1) does not translate any IPv4 options.
		LOG_DBG(NAT64, "ip4 header bigger than ip6 header(s) " IPv4_BYTES_FMT " -> " IPv4_BYTES_FMT "\n", IPv4_BYTES_LE(ipv4Header->src_addr), IPv4_BYTES_LE(addr4));
		RTE_LOG(ERR, NAT64, "no support translation with ip4 header bigger than ip6 header(s)\n");
		return -1;
	}
	if (!nmbuf) {
	...
	if (new_ipv6_header->proto == IPPROTO_ICMP) {
		...
		int result = icmp_v4_to_v6(nat64_config, packet, new_ipv6_header, nat64_config->ipv6_prefixes[entry->prefix_index].prefix, entry->ip6);
		if (result) {
			LOG_DBG(NAT64, "ICMP translation failed, dropping packet\n");
			return -1;
		}
		...
		// Recalculate UDP checksum for IPv6
		udpHeader->dgram_cksum = 0;
		// Use rte_ipv6_udptcp_cksum_mbuf for more accurate checksum calculation
		udpHeader->dgram_cksum = rte_ipv6_udptcp_cksum_mbuf(mbuf, new_ipv6_header, packet->transport_header.offset);
		LOG_DBG(NAT64, "UDP ipv6 phd checksum: %x\n", rte_ipv6_phdr_cksum(new_ipv6_header, 0));
		LOG_DBG(NAT64, "UDP checksum calculated: 0x%04X\n", rte_be_to_cpu_16(udpHeader->dgram_cksum));
...
nat64_handle_packets(
	...
	while ((packet = packet_list_pop(&packet_front->input)) != NULL) {
		...
		if (result) {
			LOG_DBG(NAT64, "Dropping packet due to translation failure\n");
