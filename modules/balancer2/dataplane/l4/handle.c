...
service_lookup(
	...
	if (is_ipv6) {
		filter_query(
			ADDR_OF(&context->config->ipv6_vs_matcher),
			ipv6_vs_matcher,
			packets,
			vs_results,
			packets_count
		...
		filter_query(
			ADDR_OF(&context->config->ipv4_vs_matcher),
			ipv4_vs_matcher,
			packets,
			vs_results,
			packets_count
	...
	struct balancer_vs *virtual_services = ADDR_OF(&context->config->vs);
...
select_reals(
	...
	for (size_t pkt_idx = 0; pkt_idx < packet_count; ++pkt_idx) {
		...
		struct balancer_real *real = select_real(
			context,
			&pkt_ctxs[pkt_idx],
			&sessions[pkt_idx],
			session_table,
		...
		pkt_ctxs[kept].selected_real_stats = real_get_stats(
...
balancer_handle_l4_packets(
	...
	struct balancer_session_table *session_table =
		ADDR_OF(&context->config->session_table);
