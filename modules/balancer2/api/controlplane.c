...
build_vs_acl(
	...
	if (rule_count == 0) {
		return filter_init(&vs->acl, compiler, NULL, 0, mctx);
	}
...
register_acl_counters(
	...
	for (size_t idx = 0; idx < source_count; ++idx) {
		...
		ids[idx] = counter_registry_register(registry, name, 1);
...
register_vs_counters(
	...
	if (register_real_counters(registry, vs, prefix, config->reals) != 0) {
		return -1;
	}
...
init_vs(struct memory_context *mctx,
	...
	vs->counter_id = (uint64_t)-1;

	vs->flags = build_vs_flags(config);

	if (build_vs_acl(mctx, vs, config) != 0) {
		return -1;
	}

	if (build_vs_reals(mctx, vs, config) != 0) {
		free_vs(mctx, vs, workers);
		return -1;
	}

	if (build_real_selector(mctx, vs, workers, config) != 0) {
		free_vs(mctx, vs, workers);
		return -1;
	}

	if (register_vs_counters(mctx, registry, vs, config) != 0) {
		free_vs(mctx, vs, workers);
		return -1;
	}
...
register_balancer_counters(
	struct balancer_module_config *cfg, struct counter_registry *registry
) {
...
build_vs_array(
	...
	for (size_t idx = 0; idx < count; ++idx) {
		if (init_vs(mctx, registry, &vs[idx], &configs[idx], workers) !=
		    0) {
...
fill_matcher_rule(
	...
	uint16_t transport = cfg->transport == tcp ? IPPROTO_TCP : IPPROTO_UDP;
...
build_vs_matcher(
	...
	for (size_t idx = 0; idx < config_count; ++idx) {
		if (configs[idx].ip_family == family) {
			++rule_count;
		}
	}
	if (rule_count == 0) {
		return filter_init(matcher, compiler, NULL, 0, mctx);
	}
	...
	for (size_t idx = 0; idx < config_count; ++idx) {
		...
		if (fill_matcher_rule(
			    &rules[filled], (uint32_t)idx, &configs[idx]
		    ) != 0) {
			goto cleanup;
		}
...
build_vs_matchers(
	...
	if (build_vs_matcher(
		    mctx, &cfg->vs_matcher_ip4, ip_family_ip4, configs, count
	    ) != 0) {
		return -1;
	}

	if (build_vs_matcher(
		    mctx, &cfg->vs_matcher_ip6, ip_family_ip6, configs, count
	    ) != 0) {
...
init_module_config(
	...
	if (cp_module_init(&cfg->cp_module, agent, "balancer", name) != 0) {
		return -1;
	}

	struct counter_registry *registry = &cfg->cp_module.counter_registry;

	if (register_balancer_counters(cfg, registry) != 0) {
		free_module_config(agent, cfg);
		return -1;
	}

	struct virtual_service *vs =
		build_vs_array(mctx, registry, vs_configs, vs_count, workers);
	if (vs == NULL) {
		free_module_config(agent, cfg);
		return -1;
	}

	if (build_vs_matchers(mctx, cfg, vs_configs, vs_count) != 0) {
		free_module_config(agent, cfg);
		return -1;
	}
...
int
balancer_session_table_chain_pop_back(struct balancer_session_table_chain *chain
) {
	uint64_t gen = rcu_load(&chain->rcu, &chain->gen);
	if ((gen & 1) == 0) {
		return -1;
