...
func setupACLFWTableHarness(tb testing.TB) (*dataplaneut.Harness, *ffi.Agent, acl.Backend) {
	...
	shm := h.SharedMemory()
	agent, err := shm.AgentAttach("acl-fwtable-test", 0, aclMemSize)
	require.NoError(tb, err)
...
// TestACL_FWTableObjectUnknownNameRejected verifies that linking a
// name with no registered object fails the config publish instead of
// silently resolving a NULL table. The failure surfaces when the module
// is wired into a chain, because that is when the object links resolve.
func TestACL_FWTableObjectUnknownNameRejected(t *testing.T) {
	_, agent, backend := setupACLFWTableHarness(t)
	...
	// Mirror wireACLPipeline, capturing the device wiring error: the
	// object links resolve when the module's ectx is built for a device
	// entry, which is the step that must reject the unknown name.
	sinkName := "acl0-sink"
	sinkHandle, err := forward.NewBackend(agent).UpdateModule(sinkName, []cforward.ForwardRule{
	...
	require.NoError(t, agent.UpdatePipeline(ffi.PipelineConfig{Name: "acl0", Functions: []string{"acl0"}}))
	require.NoError(t, agent.UpdatePipeline(ffi.PipelineConfig{Name: "dummy"}))
	err = agent.UpdatePlainDevices([]ffi.DeviceConfig{{
		Name:   "port0",
		Input:  []ffi.DevicePipelineConfig{{Name: "acl0", Weight: 1}},
		Output: []ffi.DevicePipelineConfig{{Name: "dummy", Weight: 1}},
	}})
	require.Error(t, err, "wiring a config that links an unknown object must fail")
