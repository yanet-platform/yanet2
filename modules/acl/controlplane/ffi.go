...
const (
	ActionAllow       = C.ACL_RULE_ACTION_ALLOW
	ActionDeny        = C.ACL_RULE_ACTION_DENY
	ActionCount       = C.ACL_RULE_ACTION_COUNT
	ActionCheckState  = C.ACL_RULE_ACTION_CHECK_STATE
	ActionCreateState = C.ACL_RULE_ACTION_CREATE_STATE
	ActionLog         = C.ACL_RULE_ACTION_LOG
)
...
func CBuildActions(dst *C.struct_acl_rule, actions []AclAction, pinner *runtime.Pinner) {
	...
	for idx, a := range actions {
		cActions[idx].kind = C.uint64_t(a.Kind)
	}
