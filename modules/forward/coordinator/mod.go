...
// ForwardModule represents a forward module in the coordinator.
...
// NewForwardModule creates a new instance of the forward module.
...
// Run starts the gRPC server for the module and registers it with the coordinator.
func (m *ForwardModule) Run(ctx context.Context) error {
	listener, err := net.Listen("tcp", m.endpoint)
	if err != nil {
		return fmt.Errorf("failed to create listener for forward module: %w", err)
	}

	endpoint := listener.Addr().String()
	m.log.Infow("starting forward module server", zap.String("endpoint", endpoint))
	...
	m.log.Infow("registering forward module with coordinator",
