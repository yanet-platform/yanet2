...
The existing ten generated cases remain target-only vectors: IPv4/IPv6
empty, enabled, disabled, zero-weight, and unmatched-destination states.
Together with the five source packet pairs, the matrix has 11 state cases
and 15 packet subcases. Exact VIP rules supplement the generated destination
...
Each unique Echo source seeds two UDP sessions through the normal worker
...
not a verbose per-subcase transcript.
