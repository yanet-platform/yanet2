...
func main() {
	...
	pipeline0 := C.pipeline_config_create(3)
	defer C.pipeline_config_free(pipeline0)
	C.pipeline_config_set_module(pipeline0, 0, C.CString("forward"), C.CString("to_krn"))
	C.pipeline_config_set_module(pipeline0, 1, C.CString("balancer"), C.CString("balancer0"))
	C.pipeline_config_set_module(pipeline0, 2, C.CString("route"), C.CString("route0"))
