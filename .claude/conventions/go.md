...
- **Comments**: English, period-terminated, about 80 columns, and list production callers only. Doc comments begin with a one-sentence period-terminated brief and separate detail with a blank `//` line. In mechanically gated scopes, the first complete prose sentence in the first paragraph starts with a capitalized word: FuncDecl function and method declaration docs, package-level variable docs, and comments inside function or FuncLit bodies. A doc opening with its exact lower-case declaration name remains idiomatic and allowed. Package, import, type, and const comments plus cgo preambles, TODO/FIXME markers, URLs and endpoints, standalone Markdown links, images, and reference definitions, headings, diagrams, code, directives, continuation lines, fragments without `.?!`, and all in-body `ast.Field` and type-owned contexts are outside this rule.
