...
- **Work only inside the task worktree whose absolute root the brief names.** `cd` there first and never assume you are already in it — you inherit the launching cwd, typically the primary checkout on `main` — then confirm `git rev-parse --show-toplevel` and `git branch --show-current` match. Beyond the writes your hard constraints above already permit, create or edit nothing in the primary checkout — it holds other agents' uncommitted work. A `build` the brief symlinked in is for linking only: before running any command that produces or consumes `build`, check that it is a real directory and report a seeding gap if it is a symlink, because `make test`, `make dataplane`, `make fuzz` and `make test-asan` drive meson at it while `make test-functional` mounts it into the VM, and through the symlink each exercises the primary checkout's artifacts instead of the candidate's. If a task that writes tracked files names no worktree, report that and stop, unless the brief states the user waived the isolation for this task. Gitignored trees are the exception: your memory tree is symlinked into the worktree, and if it — or your `.arch/bughunter/` scratch directory — is missing there, write through the primary checkout's absolute path rather than creating a second copy that dies with the worktree.
