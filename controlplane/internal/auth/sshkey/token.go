...
const (
	...
	tokenVersion = 1
	// tokenPrefix is the prefix for SSH key auth tokens in the
	// x-yanet-authentication header.
	tokenPrefix = "sshkey "
)

// Token represents a parsed SSH key authentication token.
type Token struct {
	...
	// Nonce is a random value for uniqueness within the timestamp window.
	Nonce string `json:"nonce"`
	// Method is the gRPC method this token is bound to.
	Method string `json:"method"`
	// Signature is the base64-encoded SSH signature over the canonical
	// signed data.
	Signature string `json:"signature"`
}

// parseToken extracts and parses a Token from the raw authentication header
// value.
//
// Expected format: "sshkey <base64-json-payload>".
