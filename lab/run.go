...
func RunManifest(fw *framework.TestFramework, path string) RunReport {
	...
	if manifest.Boot.Dataplane != "" || manifest.Boot.Controlplane != "" {
		...
		if result.Error == "" {
			if bootErr := fw.RestoreClean("preyanet"); bootErr != nil {
				result.Error = bootErr.Error()
			} else if bootErr := fw.StartYANET(dataplane, controlplane); bootErr != nil {
				result.Error = bootErr.Error()
			} else if _, bootErr := fw.ExecuteCommands(fw.CommonConfigCommands()...); bootErr != nil {
				result.Error = bootErr.Error()
			} else {
				fw.ResetConnections()
				if bootErr := fw.WaitForDatapathReady(15 * time.Second); bootErr != nil {
					result.Error = bootErr.Error()
				}
			}
		}
	...
	for _, file := range manifest.Files {
		...
		if readErr == nil {
			...
			_, readErr = fw.ExecuteCommand(command)
		}
		result.Duration = time.Since(started)
	...
	if report.Success {
		for _, step := range manifest.Steps {
			...
			output, stepErr := fw.ExecuteCommandWithTimeout(shellJoin(step.Argv), timeout)
	...
	if report.Success {
		for _, probe := range manifest.Probes {
			result := runProbe(fw, baseDir, probe)
			report.Results = append(report.Results, result)
			if !result.Success {
				report.Success = false
				break
...
func runProbe(fw *framework.TestFramework, baseDir string, probe Probe) Result {
	started := time.Now()
	...
	actual, captureErr := fw.SendPacketAndCapture(probe.Ingress, probe.Egress, packets[0], timeout)
	if probe.Expect.Drop {
		result.Success = captureErr != nil || len(actual) == 0
		if !result.Success {
			result.Error = "expected packet to be dropped, but one was captured"
		}
	} else {
