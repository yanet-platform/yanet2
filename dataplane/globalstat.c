...
static const struct {
	...
	{"nic_rx",
	 2,
	 (const size_t[]
	 ){offsetof(struct dataplane, global_stats.nic_stats.rx_count),
	   offsetof(struct dataplane, global_stats.nic_stats.rx_size)}},
	{"nic_tx",
	 2,
	 (const size_t[]
	 ){offsetof(struct dataplane, global_stats.nic_stats.tx_count),
	   offsetof(struct dataplane, global_stats.nic_stats.tx_size)}},
	{"nic_rx_tx_errors",
	 2,
	 (const size_t[]
	 ){offsetof(struct dataplane, global_stats.nic_stats.remote_rx_count),
	   offsetof(struct dataplane, global_stats.nic_stats.remote_tx_count)}},
	{"nic_rx_nombuf",
	 1,
	 (const size_t[]
	 ){offsetof(struct dataplane, global_stats.nic_stats.rx_nombuf_count)}},
};
...
uint64_t **
get_worker_field_ptr(
	struct dataplane *dataplane, size_t info_index, size_t offset_index
) {
	return (uint64_t **)((char *)dataplane + global_counter_info[info_index]
							 .offset[offset_index]);
}
...
calculate_and_update_stats(
	...
	for (uint16_t idx = 0; idx < dataplane->device_count; ++idx) {
		...
		rx_nombuf +=
			cur_stats[idx].rx_nombuf - prev_stats[idx].rx_nombuf;
	}
...
stat_nic_thread(void *arg) {
	...
	for (size_t i = 0; i < ARRAY_SIZE(global_counter_info); ++i) {
		for (size_t j = 0; j < global_counter_info[i].size; ++j) {
			uint64_t **field_ptr =
				get_worker_field_ptr(dataplane, i, j);
			*field_ptr =
				counter_get_address(
	...
	for (uint16_t idx = 0; idx < dataplane->device_count; ++idx) {
		rte_eth_stats_get(
			dataplane->devices[idx].port_id, &stats_prev[idx]
		);
	}
	...
	while (1) {
		...
		for (uint16_t idx = 0; idx < dataplane->device_count; ++idx) {
			rte_eth_stats_get(
				dataplane->devices[idx].port_id, &stats_cur[idx]
			);
		}
