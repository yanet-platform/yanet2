...
impl Display for Prefix {
    fn fmt(&self, f: &mut Formatter) -> Result<(), fmt::Error> {
        ...
        if output::is_colored() {
            if *is_best {
                if *ecmp_size > 1 {
                    write!(f, "{} {}", s, format!("ECMP×{ecmp_size}").green())
                } else {
                    write!(f, "{s}")
                }
            } else {
...
///
/// An ECMP group is the set of best routes sharing the same prefix string.
/// When a prefix has more than one best route, the `ecmp_size` field of each
/// best `Prefix` in that group is set to the group count; entries that are not
/// best, or whose prefix has only one best route, retain size 1 (unmarked).
fn annotate_ecmp_groups(entries: &mut [RouteEntry]) {
    let mut best_counts: HashMap<String, usize> = HashMap::new();

    for entry in entries.iter() {
        if entry.prefix.1 {
            *best_counts.entry(entry.prefix.0.to_string()).or_insert(0) += 1;
        }
    }

    for entry in entries.iter_mut() {
        if entry.prefix.1 {
            let count = best_counts.get(&entry.prefix.0.to_string()).copied().unwrap_or(1);
...
mod test {
    ...
    fn make_entry(prefix_str: &str, is_best: bool) -> RouteEntry {
    ...
    fn annotate_ecmp_groups_marks_multi_best_prefixes() {
        let mut entries = vec![
            make_entry("10.0.0.0/8", true),
            make_entry("10.0.0.0/8", true),
            make_entry("192.168.0.0/24", true),
            make_entry("192.168.0.0/24", false),
            make_entry("172.16.0.0/12", true),
        ];
