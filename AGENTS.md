...
- `common/` — shared code: `go/` (`xcfg`, `xcmd`, `xerror`, `xpacket`, `xgrpc`, `logging`, `metrics`, `readiness`, `testutils`, …), `rust/` (`commonpb`, `filterpb`, `ynpb`), `commonpb/`, `filterpb/`, `ttlmap/`, C headers (`lpm.h`, `radix.h`, `hash.h`, `rcu.h`, `memory*.h`).
