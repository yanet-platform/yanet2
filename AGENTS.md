...
- **This rule governs comments that explain the code to a reader, not one serving another job entirely** — a cgo preamble, a `//go:build`/`//go:generate` directive, a lint pragma, a compiler-recognized marker such as a fallthrough comment, a license header, or a generated-file marker — **each of which is out of scope and unbounded in length.**
- **No comment on a private symbol.** Private means an unexported Go identifier, a non-`pub` Rust item, or a non-exported TypeScript declaration — covers functions, methods, types, fields, constants, and variables alike. In C, private means `static` in a `.c` file; a `static`/`static inline` symbol in a header is that header's public API, not private.
- **No comment inside a function or method body.**
- **One line is the ceiling, not a floor.** Every comment an agent writes — public doc comment, package/file doc, or test comment — is at most one line. A public symbol is not required to carry a doc comment.
- **Anything beyond the private-symbol ban, the body-comment ban, and the one-line ceiling defaults to no.** A second line, a comment on a private symbol, or a body comment ships only after the user explicitly approves it in the current task. Silence is refusal, not permission, and an unapproved one is a blocking review finding.
- **Existing comments are out of scope.** Never reflow, re-wrap, re-paragraph, or re-cut a comment the change does not require. Correct one only when the change makes it false — the one-line ceiling binds what an agent writes, not what it corrects — and never cut away a load-bearing rationale.
- **A comment states a true, load-bearing fact.** Check the claim against the code before shipping it, drop an exclusivity word (`only`, `never`, `always`) the code does not guarantee, and skip what the code already shows. Present tense, human prose, no provenance or diff narration — that belongs in the pull request body.
...
- **Require whole-class audits.** A `file:line` sample is not the population: pair it with the defining grep. Map all sites and callers before modifying shared primitives, return values, or a named data-flow site. The repo-search tooling is gitignore-aware, so a plain repo-wide grep silently returns nothing for generated files or gitignored private trees. Any "no callers left", "nothing else uses this", or whole-population conclusion is therefore false until re-run naming the private and generated paths explicitly.
- **Review the complete candidate with `review-change`.** Give the reviewer the task brief, exact base, candidate worktree or PR, and full intended manifest, naming untracked and ignored paths. Classify every entry as publish or local-only and stage only the publish manifest. Keep the first pass independent of existing review comments. Record the approval fingerprint (base, manifests, modes/types, content, plus a PR's head SHA), recheck it immediately before the verdict, and restart the whole candidate on any change during review. An edit made after an approval instead re-derives the regions it changed, plus any unchanged region whose collaborator it changed, and carries the rest — the fingerprint stays per entry and is still recomputed, only the re-derivation narrows. Stage/commit/amend/push may carry approval only after proving the publish fingerprint identical on the packaged side and local-only entries unchanged. A new commit SHA alone is not a content change.
- **Name the task worktree's absolute root in every brief, and tell the agent to `cd` there first.** Agents inherit cwd, and a worktree lacks gitignored state such as `.arch/`, `.agent-state/agent-memory/`, and `.claude/agent-memory/`, so seed or symlink whatever the agent's gate needs before launching it. Salvage both memory trees before removing a worktree, and establish each participant's tree before disputing file existence.
- **Check open PRs and their diffs before briefing.** If one owns the zone, drive or review it rather than competing.
...
- **A review finding binds, its suggested remedy does not, and binding is not blocking.** Diagnose the defect and choose the minimal fix. Route a non-blocking finding by class, not into its own round. Reconsider the premise when the remedy is disproportionate.
- **After a design pivot, re-derive inherited constraints.** Retain no dead or mutually unsatisfiable requirement.
- **Route non-application work to `coder-c` with a charter note.** State that ops/packaging/CI work is not C and no C/Go/Rust/TS/proto/meson source changes. Never route infrastructure to `coder-ui`. For untracked/gitignored trees, have reviewers inspect explicit on-disk paths instead of `git diff`.
