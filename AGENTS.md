...
- **Comments: structure and scope.** A doc comment opens with a one-line brief, then a blank comment line and concise detail, and single-sentence and inline comments are exempt from that shape. This applies to `//`, `///`, `#`, and `/* */`. Write only what a caller needs at the call site, skipping what the code or a test's own name already shows, and leave the change's narrative in the pull request body. That bounds what a comment says, not whether it states a guarantee the rationale bullet requires. Cramming several ideas into one comment is blocking whether it is a doc comment or inline, and re-paragraphing is not the fix when the result is longer — cut ideas instead.
...
- **A more specific claim is falsified before it ships.** Replacing silence or vague prose with a precise statement — in first-pass documentation as much as in a review-driven rewrite — means checking it against the code it describes, including a dependency's real defaults and everything the subject reaches through its collaborators. Where the check does not hold, drop the exclusivity word — for example `only`, `never`, `reserved`, or a load-bearing definite article — and state what the code does: a rewrite a reader falsifies in one lookup is worse than the vague original. Across review rounds, prefer weakening the claim to appending another counterexample or qualifier — a caveat added each round grows the comment without bound, while a weaker claim that stays true costs nothing further. Rewriting an already-corrected false clause is the highest-risk case.
