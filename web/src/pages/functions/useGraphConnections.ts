...
/**
 * Hook for managing graph connections and validation
 */
export const useGraphConnections = (graph: Graph) => {
    ...
    const validateConnection = useCallback((
        ...
        // Rule 3: Regular blocks (non-terminal) can have only 1 outgoing connection
        ...
        // Rule 4: Regular blocks (non-terminal) can have only 1 incoming connection
        // OUTPUT block can have multiple incoming connections (for multiple chains)
        if (targetBlockId !== 'output' && hasIncomingConnection(targetBlockId)) {
