...
const BulkBar: React.FC<{
    ...
    <div className="fwng-bulk-bar">
        <span className="fwng-bulk-bar__count">{count} selected</span>
        <button type="button" className="fwng-btn fwng-btn--danger fwng-btn--sm" onClick={onDelete}>
            Delete
        </button>
        <button type="button" className="fwng-icon-btn fwng-icon-btn--sm" onClick={onClear} aria-label="Clear selection">
            ✕
        </button>
    </div>
);
...
const ConfigTabStrip: React.FC<{
    ...
    <div className="fwng-tabs" role="tablist">
        {configs.map((cfg) => (
            <button
                key={cfg}
                type="button"
                ...
                className={`fwng-tab${cfg === activeConfig ? ' fwng-tab--active' : ''}${dirtyConfigs.has(cfg) ? ' fwng-tab--dirty' : ''}`}
                onClick={() => onSelect(cfg)}
            >
                <span className="fwng-tab__label">{cfg}</span>
                {dirtyConfigs.has(cfg) && (
                    <span className="fwng-tab__dot" aria-label="unsaved changes" />
                )}
                <span className="fwng-tab__count">{ruleCounts.get(cfg) ?? 0}</span>
            </button>
        ))}
        <Button view="flat" size="s" onClick={onAddConfig} className="fwng-tabs__add" title="Add config">
            <Icon data={Plus} size={14} />
        </Button>
    </div>
);

const ForwardNgPage: React.FC = () => {
    const {
        draftConfigs,
        loading,
        draftRules,
    ...
    const pageHeader = (
        <Flex alignItems="center" gap={4} style={{ width: '100%' }}>
            <Text variant="header-1">Forward (NG)</Text>
            <Flex grow />
    ...
    return (
        <PageLayout header={pageHeader}>
            <div className="fwng-page">
                {draftConfigs.length === 0 ? (
                    <div className="fwng-empty-page">
                        <div className="fwng-empty-page__message">
                ...
                {deleteConfirmOpen && (
                    <div className="fwng-modal-backdrop" onClick={() => setDeleteConfirmOpen(false)}>
                        <div className="fwng-modal fwng-modal--sm" onClick={(e) => e.stopPropagation()}>
                            <header className="fwng-modal__head">
                                <span className="fwng-modal__title">Delete rules</span>
                                <button type="button" className="fwng-icon-btn" onClick={() => setDeleteConfirmOpen(false)} aria-label="Close">✕</button>
                            </header>
                            <div className="fwng-modal__body fwng-modal__body--confirm">
                            ...
                            <footer className="fwng-modal__foot">
                                ...
                                <div className="fwng-modal__foot-actions">
                                    <button type="button" className="fwng-btn fwng-btn--ghost" onClick={() => setDeleteConfirmOpen(false)}>Cancel</button>
                                    <button type="button" className="fwng-btn fwng-btn--danger" onClick={handleBulkDelete}>
                ...
                {addConfigOpen && (
                    <div className="fwng-modal-backdrop" onClick={() => setAddConfigOpen(false)}>
                        <div className="fwng-modal fwng-modal--sm" onClick={(e) => e.stopPropagation()}>
                            <header className="fwng-modal__head">
                                <span className="fwng-modal__title">Add config</span>
                                <button type="button" className="fwng-icon-btn" onClick={() => setAddConfigOpen(false)} aria-label="Close">✕</button>
                            </header>
                            <div className="fwng-modal__body fwng-modal__body--confirm">
                                <div className="fwng-field">
                                    <label className="fwng-field__label" htmlFor="fwng-new-config-name">
                                        Config name <span className="fwng-field__req">*</span>
                                    </label>
                                    <input
                                        id="fwng-new-config-name"
                            ...
                            <footer className="fwng-modal__foot">
                                <span />
                                <div className="fwng-modal__foot-actions">
                                    <button type="button" className="fwng-btn fwng-btn--ghost" onClick={() => setAddConfigOpen(false)}>Cancel</button>
                                    <button
                                        type="button"
                ...
                {deleteConfigOpen && (
                    <div className="fwng-modal-backdrop" onClick={() => setDeleteConfigOpen(false)}>
                        <div className="fwng-modal fwng-modal--sm" onClick={(e) => e.stopPropagation()}>
                            <header className="fwng-modal__head">
                                <span className="fwng-modal__title">Delete config</span>
                                <button type="button" className="fwng-icon-btn" onClick={() => setDeleteConfigOpen(false)} aria-label="Close">✕</button>
                            </header>
                            <div className="fwng-modal__body fwng-modal__body--confirm">
                            ...
                            <footer className="fwng-modal__foot">
                                <span />
                                <div className="fwng-modal__foot-actions">
                                    <button type="button" className="fwng-btn fwng-btn--ghost" onClick={() => setDeleteConfigOpen(false)}>Cancel</button>
                                    <button type="button" className="fwng-btn fwng-btn--danger" onClick={handleDeleteConfig}>
                                        Mark for deletion
                                    </button>
                                </div>
                            </footer>
