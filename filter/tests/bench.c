...
gen_packets(size_t count) {
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        packets[i] = make_packet(ip(src_ip, 1, 1, 5), ip(dst_ip, 2, 3, 1), src_port, dst_port, IPPROTO_UDP, 0, 0);
    }
    return packets;
}
...
int
query_filter_compiler(struct filter_compiler *filter_compiler, struct packet *packet, uint32_t *rule_count,
    uint32_t **rules) {
	...
	uint32_t src_port = value_table_get(
			&filter_compiler->src_port4, 0,
		packet_src_port(packet)
    );
    uint32_t dst_port = value_table_get(
			&filter_compiler->dst_port4, 0,
		packet_dst_port(packet)
    );
	...
	uint32_t result =
		value_table_get(&filter_compiler->v4_lookups.result, net, transport);

	struct value_range *range =
		ADDR_OF(&filter_compiler->v4_lookups.result_registry.ranges) + result;
	*rules = ADDR_OF(&filter_compiler->v4_lookups.result_registry.values) + range->from;
...
main() {
    ...
    struct filter_attribute attributes[4] = {
        attribute_net4_src, 
        attribute_net4_dst,
        attribute_port_src, 
        attribute_port_dst
    ...
    // generate rules
    int g = 0;
    struct filter_rule *rules = malloc(sizeof(struct filter_rule) * MAX_IP * MAX_IP);
    struct filter_rule_builder *builders = malloc(sizeof(struct filter_rule_builder) * MAX_IP * MAX_IP);
    for (int i = 0; i < MAX_IP; ++i) {
        for (int j = 0; j < MAX_IP; ++j) {
            ...
            struct filter_rule_builder *builder = &builders[i * MAX_IP + j];
            builder_init(builder);
            builder_add_port_src_range(builder, src_port1, src_port2);
            builder_add_port_dst_range(builder, dst_port1, dst_port2);
    ...
    res = filter_init(&filter, attributes, 4, rules, MAX_IP * MAX_IP, &memory_context);
    assert(res == 0);
    double filter_init_time = (double)((clock() - new_filter_init_start_time)) / CLOCKS_PER_SEC;
    ...
    res = filter_compiler_init(&filter_compiler, &memory_context1, rules, MAX_IP * MAX_IP);
    assert(res == 0);
    double old_filter_init_time = (double)((clock() - old_filter_init_start_time)) / CLOCKS_PER_SEC;
    ...
    double new_filter_query_time = (double)((clock() - filter_query_start_time)) / CLOCKS_PER_SEC;
    printf("New filter summary query time: %.4f seconds (%.2f mp/s)\n", new_filter_query_time, (double)PACKETS / new_filter_query_time / 1e6);
    ...
    for (size_t i = 0; i < PACKETS; ++i) {
        uint32_t *actions;
        uint32_t actions_count;
        query_filter_compiler(&filter_compiler, &packets[i], &actions_count, &actions);
        old_filter_checksum ^= actions_count;
        for (size_t j = 0; j < actions_count; ++j) {
            old_filter_checksum ^= actions[j];
        }
    }
    double old_filter_query_time = (double)((clock() - old_filter_query_start_time)) / CLOCKS_PER_SEC;
    printf("Old filter summary query time: %.4f seconds (%.2f mp/s)\n", old_filter_query_time, (double)PACKETS / old_filter_query_time / 1e6);
