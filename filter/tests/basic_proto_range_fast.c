...
FILTER_COMPILER_DECLARE(sign_proto_range_fast, proto_range_fast);
...
test_basic_tcp_udp(void *memory) {
	...
	struct filter_rule r1 = build_rule(&b1, 1 | ACTION_NON_TERMINATE);
	...
	struct filter_rule r2 = build_rule(&b2, 2 | ACTION_NON_TERMINATE);
	...
	res = FILTER_INIT(
		&filter, sign_proto_range_fast, rules, 2, &memory_context
	);
	TEST_ASSERT_EQUAL(res, 0, "failed to initialize filter");

	LOG(INFO, "query tcp packet...");
	query_tcp_packet(&filter, 0, 1 | ACTION_NON_TERMINATE);

	LOG(INFO, "query udp packet...");
	query_udp_packet(&filter, 2 | ACTION_NON_TERMINATE);

	FILTER_FREE(&filter, sign_proto_range_fast);
...
test_tcp_flags(void *memory) {
	...
	struct filter_rule r3 = build_rule(&b3, 3 | ACTION_NON_TERMINATE);

	struct filter_rule rules[3] = {r1, r2, r3};

	struct filter filter;
	res = FILTER_INIT(
		&filter, sign_proto_range_fast, rules, 3, &memory_context
	);
	TEST_ASSERT_EQUAL(res, 0, "failed to initialize filter");

	LOG(INFO, "query tcp SYN packet...");
	query_tcp_packet(&filter, 0x02, 1 | ACTION_NON_TERMINATE);

	LOG(INFO, "query tcp ACK packet...");
	query_tcp_packet(&filter, 0x10, 2 | ACTION_NON_TERMINATE);

	LOG(INFO, "query tcp FIN packet...");
	query_tcp_packet(&filter, 0x01, 3 | ACTION_NON_TERMINATE);
...
test_overlapping_ranges(void *memory) {
	...
	// Rule 1: All TCP (256*6 to 256*6+255)
	struct filter_rule_builder b1;
	builder_init(&b1);
	builder_add_proto_range(
		&b1, 256 * IPPROTO_TCP, 256 * IPPROTO_TCP + 255
	...
	// Rule 2: TCP SYN only (overlaps with rule 1)
	struct filter_rule_builder b2;
	builder_init(&b2);
	builder_add_proto_range(
		&b2, 256 * IPPROTO_TCP + 0x02, 256 * IPPROTO_TCP + 0x02
	);
	struct filter_rule r2 = build_rule(&b2, 2 | ACTION_NON_TERMINATE);
	...
	// TCP SYN should match both rules
	struct packet packet = {0};
	uint8_t sip[NET4_LEN] = {0, 0, 0, 0};
	uint8_t dip[NET4_LEN] = {0, 0, 0, 0};
	res = fill_packet_net4(&packet, sip, dip, 0, 0, IPPROTO_TCP, 0x02);
	...
	TEST_ASSERT_EQUAL(
		actions->count, 2, "expected 2 matching rules for TCP SYN"
	);
	TEST_ASSERT_EQUAL(
		ADDR_OF(&actions->values)[0],
		1 | ACTION_NON_TERMINATE,
		"first match should be rule 1"
	);
	TEST_ASSERT_EQUAL(
		ADDR_OF(&actions->values)[1],
		2 | ACTION_NON_TERMINATE,
		"second match should be rule 2"
	);
...
test_multiple_ranges_per_rule(void *memory) {
	...
	struct filter filter;
	res = FILTER_INIT(
		&filter, sign_proto_range_fast, rules, 1, &memory_context
	);
	TEST_ASSERT_EQUAL(res, 0, "failed to initialize filter");
	...
	LOG(INFO, "query udp packet...");
	query_udp_packet(&filter, 1 | ACTION_NON_TERMINATE);
...
test_boundary_values(void *memory) {
	...
	TEST_ASSERT_EQUAL(
		ADDR_OF(&actions1->values)[0],
		1 | ACTION_NON_TERMINATE,
		"proto 0 should match rule 1"
	);
	free_packet(&packet1);
...
main() {
	...
	++tests;
	if (test_overlapping_ranges(memory) != 0) {
