...
void query_and_expect_actions(struct filter *filter, uint16_t src_port, uint16_t dst_port, uint32_t action_count, uint32_t *actions) {
    struct packet packet = make_packet(123, 321, src_port, dst_port, IPPROTO_UDP, 0, 0);
...
void test1(void *memory) {
    ...
    const struct filter_attribute *attrs[2] = {&attribute_port_src, &attribute_port_dst};
    ...
    // query packet which corresponds to all rules
    {
        ...
        uint32_t ref_actions[3] = {ACTION_NON_TERMINATE | 1, ACTION_NON_TERMINATE | 2, ACTION_NON_TERMINATE | 3};
        query_and_expect_actions(&filter, src_port, dst_port, 3, ref_actions);
    }
    ...
    // query packet which corresponds to rules 1 and 3
    {
        ...
        const uint16_t dst_port = 460;
        uint32_t ref_actions[2] = {ACTION_NON_TERMINATE | 1, ACTION_NON_TERMINATE | 3};
        query_and_expect_actions(&filter, src_port, dst_port, 2, ref_actions);
    }
    ...
    // query packet which corresponds to rules 2 and 3
    {
        ...
        const uint16_t dst_port = 460;
        uint32_t ref_actions[2] = {ACTION_NON_TERMINATE | 2, ACTION_NON_TERMINATE | 3};
        query_and_expect_actions(&filter, src_port, dst_port, 2, ref_actions);
    }
    ...
    // query packet which corresponds to rule 1 only
    {
        ...
        query_and_expect_actions(&filter, src_port, dst_port, 1, ref_actions);
    }
...
void test2(void *memory) {
    ...
    // query packet which corresponds to all rules, but only 1, 2, 3 are returned
    {
        const uint16_t src_port = 110;
        const uint16_t dst_port = 460;
        uint32_t ref_actions[3] = {ACTION_NON_TERMINATE | 1, ACTION_NON_TERMINATE | 2, 3};
        query_and_expect_actions(&filter, src_port, dst_port, 3, ref_actions);
    }

    // query packet which corresponds to rules 1, 3 and 4, but only 1 and 3 are returned
    {
        const uint16_t src_port = 190;
        const uint16_t dst_port = 460;
    ...
    // query packet which corresponds to rules 2, 3 and 4, byt only 2 and 3 are returned
    {
        const uint16_t src_port = 60;
        const uint16_t dst_port = 460;
    ...
    // query packet which corresponds to rules 3 and 4, but only 3 is returned
    {
        const uint16_t src_port = 20;
        const uint16_t dst_port = 500;
