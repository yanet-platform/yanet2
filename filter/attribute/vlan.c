...
init_vlan(
    ...
    struct value_table *t = memory_balloc(memory_context, sizeof(struct value_table));
    if (t == NULL) {
        return -1;
    }
    ...
    *data = t;
    ...
    for (const struct filter_rule *r = rules; r < rules + rule_count; ++r) {
        ...
        if (r->vlan == VLAN_UNSPEC) {
            ...
            value_registry_collect(registry, value_table_get(t, 0, r->vlan));
        }
    }
    return 0;
}
...
struct filter_attribute attribute_vlan = {
    init_vlan,
    lookup_vlan,
    free_vlan
};