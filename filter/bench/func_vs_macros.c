...
static uint32_t
test_macro_filter(struct filter_rule *rules, struct packet *packets) {
	...
	int res =
		memory_context_init(&memory_context, "test_macro", &allocator);
	assert(res == 0);
...
static uint32_t
test_func_filter(struct filter_rule *rules, struct packet *packets) {
	struct block_allocator allocator;
	block_allocator_init(&allocator);
	void *memory = malloc(MEMORY);
	block_allocator_put_arena(&allocator, memory, MEMORY);
...
main() {
	...
	uint32_t ck1 = test_macro_filter(rules, packets);
	uint32_t ck2 = test_func_filter(rules, packets);
