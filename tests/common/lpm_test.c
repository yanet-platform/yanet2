...
test_overlapping_prefixes(void) {
	...
	struct lpm tree;
	if (lpm_init(&tree, &mctx, "lpm", NULL)) {
		free(arena);
		return -1;
	}
	...
	memset(from, 0x00, 4);
	memset(to, 0xff, 4);
	lpm_insert(&tree, 4, from, to, 42, NULL);
	from[1] = 0x01;
	to[0] = 0x00;
	to[1] = 0x01;
	lpm_insert(&tree, 4, from, to, 99, NULL);
...
test_lookup_batch_matches_scalar(void) {
	...
	for (size_t ks = 0; ks < sizeof(key_sizes) / sizeof(key_sizes[0]);
		...
		for (size_t i = 0; i < 300; i++) {
			...
			if (lpm_insert(&tree, key_size, from, to, i, NULL)) {
				lpm_free(&tree);
				goto out;
			}
		}
...
main(int argc, char **argv) {
	...
	do {
		...
		to[15] = 8;
		if (lpm_insert(&lpm, 16, from, to, idx, NULL)) {
			break;
		}
		++idx;
	} while (1);
