...
int
stupid_init(struct stupid *stupid, struct memory_context *mctx, uint64_t now, uint64_t max_time) {
...
static inline void
stupid_put(struct stupid *stupid, uint32_t from, uint32_t timeout, int32_t cnt) {
...
int stress(void *arena, uint64_t now, uint64_t max_time, uint64_t max_timeout) {
...
int main() {
    ...
    TEST_ASSERT_EQUAL(test_basic_result, TEST_SUCCESS, "test 'basic' failed");
    free(arena);
