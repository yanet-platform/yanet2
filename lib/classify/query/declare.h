 ...
 * written in the order of the classifier tape leaves:
 *
 *	static const struct classify_query_attr_handlers *my_query[] = {
 *		&my_attr_device,
 *		&my_attr_vlan,
 *	};
 *	classify_query(filter, my_query, packets, results, count);
 */
#define CLASSIFY_QUERY_ATTR(tag, fn)                                           \
