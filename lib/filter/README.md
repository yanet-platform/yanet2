...
- Query
  - Interface: lib/filter/query.h
  - Macros: FILTER_QUERY_DECLARE, FILTER_QUERY

- Compiler
  - Interface: lib/filter/compiler.h (+ implementation under lib/filter/compiler/*.c)
  - Macros: FILTER_COMPILER_DECLARE, FILTER_INIT, FILTER_FREE

## Files

- Public interfaces:
  - lib/filter/rule.h — rule data types and action encoding (inputs to compiler, visible to query)
  - lib/filter/compiler.h — build (compile) filter for a given signature
  - lib/filter/query.h — query filter and post-process actions
  - lib/filter/filter.h — core types (struct filter, vertices, tables, registries)
...
- Query (header-only):
  ...
  - Interface headers: [`lib/filter/query.h`](lib/filter/query.h) and [`lib/filter/filter.h`](lib/filter/filter.h)
  - Meson dependency: [`lib_filter_query_dep`](lib/filter/meson.build:1) (include-only, no link objects)
- Compiler:
  - Use for controlplane to build the filter instance from rules
  - Interface: [`lib/filter/compiler.h`](lib/filter/compiler.h) (implementation under `lib/filter/compiler/*.c`)
  - Meson target/dep: [`lib_filter_compiler`](lib/filter/meson.build:16) / [`lib_filter_compiler_dep`](lib/filter/meson.build:25)
...
See the library declarations in [`lib/filter/meson.build`](lib/filter/meson.build:1).

### Typical flow in this codebase

1) Controlplane prepares rules and builds a filter:
   - Declare signature once with macros:
     - Compiler side: [`FILTER_COMPILER_DECLARE`](lib/filter/compiler/declare.h) and then build via [`FILTER_INIT`](lib/filter/compiler.h:28)
     - Query side (for consumers): [`FILTER_QUERY_DECLARE`](lib/filter/query/attribute.h)
   - Persist or share the built filter (for example via shared memory as in shm tests)

2) Dataplane queries the filter:
   - Include query headers and declare the same signature order with [`FILTER_QUERY_DECLARE`](lib/filter/query/attribute.h)
   - On packet path, call [`FILTER_QUERY`](lib/filter/query.h:119) to obtain actions slice (owned by filter memory)
...
- Query is header-only; compiler links implementation objects from `lib/filter/compiler/*.c`.
...
`lib/filter/tests/helpers.h` provides a small rule builder surface used in unit tests:
