...
#define FILTER_U16_RANGES_DECLARE(tag, query_type, query_free)                 \
	struct filter_compile_attr_##tag {                                     \
	...
	filter_compile_attr_##tag##_create(                                    \
		...
		const struct filter_compile_u16_handlers *u16_handlers =       \
			container_of(                                          \
				...
				attr_handlers                                  \
                                                                               ...
                                                                               \
		struct filter_compile_attr_##tag *attr =                       \
			(struct filter_compile_attr_##tag *)memory_balloc(     \
				memory_context,                                \
				sizeof(struct filter_compile_attr_##tag)       \
			);                                                     \
		if (attr == NULL) {                                            \
			return NULL;                                           \
		}                                                              \
	...
	error_free:                                                            \
		...
		return NULL;                                                   \
                                                                               ...
                                                                               \
	static inline uint32_t filter_compile_attr_##tag##_size(               \
		const struct filter_compile_attr *attr                         \
	) {                                                                    \
		struct filter_compile_attr_##tag *u16_attr = container_of(     \
			attr, struct filter_compile_attr_##tag, attr           \
		);                                                             \
                                                                               \
		return u16_attr->value_table.v_dim *                           \
		       u16_attr->value_table.h_dim;                            \
                                                                               ...
                                                                               \
	static inline int filter_compile_attr_##tag##_rule_is_any(             \
		const struct filter_compile_attr *attr,                        \
		const struct filter_compile_attr_handlers *attr_handlers,      \
		...
		return ranges.count == 0 ||                                    \
		       ranges.items[0].to - ranges.items[0].from == 65535;     \
                                                                               ...
                                                                               \
	static inline uint32_t filter_compile_attr_##tag##_hash(               \
		const struct filter_compile_attr_handlers *attr_handlers,      \
		const struct filter_rule *rule                                 \
	) {                                                                    \
		const struct filter_compile_u16_handlers *u16_handlers =       \
		...
		return hash;                                                   \
                                                                               ...
                                                                               \
	static inline int filter_compile_attr_##tag##_compare(                 \
		const struct filter_compile_attr_handlers *attr_handlers,      \
		...
		return memcmp(first_ranges.items,                              \
			      ...
			      first_ranges.count *                             \
				      sizeof(*first_ranges.items)) != 0;       \
                                                                               ...
                                                                               \
	static inline int filter_compile_attr_##tag##_rule_iter(               \
		struct filter_compile_attr *attr,                              \
		const struct filter_compile_attr_handlers *attr_handlers,      \
		const struct filter_rule *rule,                                \
		...
		return 0;                                                      \
                                                                               ...
                                                                               \
	static inline int filter_compile_attr_##tag##_iter(                    \
		struct filter_compile_attr *attr,                              \
		const struct filter_compile_attr_handlers *attr_handlers,      \
		filter_compile_attr_iter_cb_func iter_cb_func,                 \
		void *cb_func_data                                             \
		...
		return 0;                                                      \
                                                                               ...
                                                                               \
	static inline void filter_compile_attr_##tag##_free(                   \
		struct memory_context *memory_context,                         \
		struct filter_compile_attr *attr                               \
	) {                                                                    \
		struct filter_compile_attr_##tag *u16_attr = container_of(     \
	...
	filter_compile_attr_##tag##_commit(                                    \
		struct memory_context *memory_context,                         \
		struct filter_compile_attr *attr                               \
	) {                                                                    \
		struct filter_compile_attr_##tag *u16_attr = container_of(     \
		...
		while (commit_ctx.pos < 65536) {                               \
			*vline_get_ptr(&query_attr->line, commit_ctx.pos++) =  \
				commit_ctx.value;                              \
                                                                               ...
                                                                               \
		filter_compile_attr_##tag##_free(memory_context, attr);        \
                                                                               \
		return &query_attr->attr;                                      \
	}                                                                      \
                                                                               \
	static const struct filter_compile_attr_handlers                       \
		filter_compile_attr_##tag##_handlers = {                       \
			.create = filter_compile_attr_##tag##_create,          \
			.size = filter_compile_attr_##tag##_size,              \
			.iter = filter_compile_attr_##tag##_iter,              \
			.rule_is_any =                                         \
				filter_compile_attr_##tag##_rule_is_any,       \
			.hash = filter_compile_attr_##tag##_hash,              \
			.compare = filter_compile_attr_##tag##_compare,        \
			.rule_iter = filter_compile_attr_##tag##_rule_iter,    \
			.commit = filter_compile_attr_##tag##_commit,          \
			.free_compile = filter_compile_attr_##tag##_free,      \
