...
// The 16 bit range attributes share a region based builder; the declare
// form is emitted by their family headers (u16_ranges.h).
#define FILTER_COMPILE_ATTR_U16_RANGES_BUILD_AS_DECLARE(variant)               \
	static inline struct filter_query_attr *                               \
	filter_compile_attr_##variant##_build(                                 \
		struct value_registry *registry,                               \
		const struct filter_rule **rules,                              \
